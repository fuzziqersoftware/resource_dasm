...
void decode_pict(const string& out_dir, const void* data, size_t size,
    uint32_t type, int16_t id) {
...
void decode_snd(const string& out_dir, const void* data, size_t size,
    uint32_t type, int16_t id) {
...
void decode_strN(const string& out_dir, const void* data, size_t size,
    uint32_t type, int16_t id) {
...
typedef void (*resource_decode_fn)(const string& out_dir, const void* data,
...
int main(int argc, char* argv[]) {
  ...
  filename = first_file_that_exists({
  ...
  for (const auto& it : enum_file_resources(filename.c_str())) {
    ...
    size_t size;
    try {
      load_resource_from_file(filename.c_str(), it.first, it.second, &data,
          &size);
