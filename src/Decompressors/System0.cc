...
static const vector<uint16_t> const_table({
  // 4B
...
string decompress_system0(const string& data, size_t decompressed_size) {
  ...
  for (;;) {
    ...
    if (command == 0) { // <size> <data> - raw data; size is in words
      ...
      w.write(memo.at(r.get_u8() + 0x0128));
    } else if (command == 0x22) { // <slot16> - write memo string, slot + 0x28
      // TODO: It's a little weird that this isn't +0x228. Is +0x28 correct?
      ...
      w.put_u16r(const_table.at(command - 0x4B));
      ...
      switch (r.get_u8()) {
        case 0: { // <segnum> <count-1> <index>... - export table
          ...
          uint32_t count = read_encoded_int(r);
        ...
        case 1: { // <tgoff> <a5dlt> <count-1> <a5off> - jump table
          ...
          uint32_t count = read_encoded_int(r) + 1;
        ...
        case 2: { // <value> <count> - run-length encoded bytes
          ...
          for (uint32_t count = read_encoded_int(r) + 1; count; count--) {
            w.put_u8(v);
          }
          break;
        }
