...
void ResourceFile::add_name_index_entry(std::shared_ptr<Resource> res) {
  if (!res->name.empty()) {
    this->name_to_resource.emplace(res->name, res);
  }
}

void ResourceFile::delete_name_index_entry(std::shared_ptr<Resource> res) {
  if (!res->name.empty()) {
...
uint64_t ResourceFile::make_resource_key(uint32_t type, int16_t id) {
  // Map resource ID from -32768..32767 to 0..65535, so for the same type, keys are sorted by the id (otherwise keys
  // with negative ids would compare greater-than keys with positive ids)
...
ResourceFile::Resource::Resource(uint32_t type, int16_t id, std::string&& data)
    : type(type), id(id), flags(0), data(std::move(data)) {}

ResourceFile::Resource::Resource(
    uint32_t type, int16_t id, uint16_t flags, const std::string& name, const std::string& data)
...
ResourceFile::Resource::Resource(uint32_t type, int16_t id, uint16_t flags, std::string&& name, std::string&& data)
    : type(type), id(id), flags(flags), name(std::move(name)), data(std::move(data)) {}
...
bool ResourceFile::add(const Resource& res_obj) {
  auto res = std::make_shared<Resource>(res_obj);
  return this->add(res);
}

bool ResourceFile::add(Resource&& res_obj) {
  auto res = std::make_shared<Resource>(std::move(res_obj));
  return this->add(res);
}

bool ResourceFile::add(std::shared_ptr<Resource> res) {
...
bool ResourceFile::rename(uint32_t type, int16_t id, const std::string& new_name) {
  if (new_name.size() > 0xFF) {
    throw std::invalid_argument("name must be 255 bytes or shorter");
  }
...
std::shared_ptr<const ResourceFile::Resource> ResourceFile::decompress_if_requested(
  ...
  if (res->flags & ResourceFlag::FLAG_COMPRESSED) {
    if (!res->decompressed_resource) {
      ...
      try {
        ...
        phosg::fwrite_fmt(stderr, "failed to decompress resource: {}\n", e.what());
        res->flags |= ResourceFlag::FLAG_DECOMPRESSION_FAILED;
        return res;
      }
    }
...
std::shared_ptr<const ResourceFile::Resource> ResourceFile::get_resource(
...
std::vector<int16_t> ResourceFile::all_resources_of_type(uint32_t type) const {
...
std::vector<uint32_t> ResourceFile::all_resource_types() const {
...
std::vector<std::pair<uint32_t, int16_t>> ResourceFile::all_resources() const {
  std::vector<std::pair<uint32_t, int16_t>> ret;
...
uint32_t ResourceFile::find_resource_by_id(int16_t id, const std::vector<uint32_t>& types) const {
  for (uint32_t type : types) {
    if (this->resource_exists(type, id)) {
      return type;
    }
  }
  throw std::runtime_error(std::format("referenced resource {} not found", id));
}
...
ResourceFile::TemplateEntry::TemplateEntry(std::string&& name, Type type, TemplateEntryList&& list_entries)
    : name(std::move(name)),
      type(type),
      format(Format::FLAG),
      width(2),
...
ResourceFile::TemplateEntryList ResourceFile::decode_TMPL(std::shared_ptr<const Resource> res) {
...
ResourceFile::TemplateEntryList ResourceFile::decode_TMPL(const void* data, size_t size) {
  ...
  std::vector<std::unique_ptr<Entry>> ret;
  std::vector<std::vector<std::unique_ptr<Entry>>*> write_stack;
  write_stack.emplace_back(&ret);
  bool in_bbit_array = false;
  while (!r.eof()) {
    std::string name = decode_mac_roman(r.readx(r.get_u8()));
    uint32_t type = r.get_u32b();

    if (write_stack.empty()) {
      throw std::runtime_error("TMPL ended list with no list open");
    }
    auto* entries = write_stack.back();

    if (in_bbit_array && type != 0x42424954) {
      throw std::runtime_error("BBIT array length is not a multiple of 8");
    }
    ...
    switch (type) {
      case 0x44564452: // DVDR; Resorcerer-only. Divider line with comment (0 bytes)
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::VOID, Format::DECIMAL, 0, 0, 0));
        break;
      case 0x43415345: { // CASE; Resorcerer-only. Symbolic and/or default value (0 bytes)
        // These appear to be of the format <name>=<value>. <value> is an integer in decimal format or hex (preceded by
        // a $).
        auto tokens = phosg::split(name, '=');
        if (tokens.size() != 2) {
          throw std::runtime_error("CASE entry does not contain exactly one '=' character");
        }
        if (tokens[1].empty()) {
          throw std::runtime_error("CASE value token is empty");
        }
        name = tokens[0];
      ...
      case 0x55425954: // UBYT; Resorcerer-only. "unsigned decimal byte"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DECIMAL, 1, 0, 0, false));
        break;
      case 0x55575244: // UWRD; Resorcerer-only. "unsigned decimal word"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DECIMAL, 2, 0, 0, false));
        break;
      case 0x554C4E47: // ULNG; Resorcerer-only. "unsigned decimal long"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DECIMAL, 4, 0, 0, false));
        break;
      case 0x44415445: // DATE; Resorcerer-only. Macintosh System Date/Time (seconds)
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DATE, 4, 0, 0, false));
        break;
      case 0x44425954: // DBYT; Resorcerer-only. "signed decimal byte"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DECIMAL, 1, 0, 0));
        break;
      case 0x44575244: // DWRD; Resorcerer-only. "signed decimal word"
      case 0x52534944: // RSID; Resorcerer-only. "resource ID"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DECIMAL, 2, 0, 0));
        break;
      case 0x444C4E47: // DLNG; Resorcerer-only. "signed decimal long"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::DECIMAL, 4, 0, 0));
        break;
      case 0x48425954: // HBYT; Resorcerer-only. "hex byte"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::HEX, 1, 0, 0));
        break;
      case 0x48575244: // HWRD; Resorcerer-only. "hex word"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::HEX, 2, 0, 0));
        break;
      case 0x484C4E47: // HLNG; Resorcerer-only. "hex long"
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::HEX, 4, 0, 0));
        break;
      case 0x46495844: // FIXD; Resorcerer-only. 16:16 Fixed Point Number.
        // .width specifies the width per component (16+16=32)
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::FIXED_POINT, Format::DECIMAL, 2, 0, 0));
        break;
      case 0x504E5420: // PNT ; Resorcerer-only. QuickDraw Point.
        // .width specifies the width per component (16+16=32)
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::POINT_2D, Format::DECIMAL, 2, 0, 0));
        break;
      case 0x41575244: // AWRD; align to 2-byte boundary
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::ALIGNMENT, Format::HEX, 0, 2, 0));
        break;
      case 0x414C4E47: // ALNG; align to 2-byte boundary
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::ALIGNMENT, Format::HEX, 0, 4, 0));
        break;
      case 0x46425954: // FBYT; fill byte
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::ZERO_FILL, Format::HEX, 1, 0, 0));
        break;
      case 0x46575244: // FWRD; fill word
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::ZERO_FILL, Format::HEX, 2, 0, 0));
        break;
      case 0x464C4E47: // FLNG; fill long
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::ZERO_FILL, Format::HEX, 4, 0, 0));
        break;
      case 0x48455844: // HEXD; hex dump
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::EOF_STRING, Format::HEX, 0, 0, 0));
        break;
      case 0x50535452: // PSTR; Pascal string
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::PSTRING, Format::TEXT, 1, 0, 0));
        break;
      case 0x57535452: // WSTR; Pascal string with word-sized length
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::PSTRING, Format::TEXT, 2, 0, 0));
        break;
      case 0x4C535452: // LSTR; Pascal string with long-sized length
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::PSTRING, Format::TEXT, 4, 0, 0));
        break;
      case 0x45535452: // ESTR; even-padded Pascal string
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::PSTRING, Format::TEXT, 1, 2, 0));
        break;
      case 0x4F535452: // OSTR; odd-padded Pascal string
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::PSTRING, Format::TEXT, 1, 2, 1));
        break;
      case 0x43535452: // CSTR; C string (null-terminated)
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::CSTRING, Format::TEXT, 1, 0, 0));
        break;
      case 0x45435354: // ECST; even-padded C string
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::CSTRING, Format::TEXT, 1, 2, 0));
        break;
      case 0x4F435354: // OCST; odd-padded C string
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::CSTRING, Format::TEXT, 1, 2, 1));
        break;
      case 0x424F4F4C: // BOOL; boolean word
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::BOOL, Format::FLAG, 2, 0, 0));
        break;
      case 0x42424954: // BBIT; bit within a word
        if (in_bbit_array) {
          ...
          entries->emplace_back(std::make_unique<Entry>("", Type::BITFIELD, Format::FLAG, 1, 0, 0));
      ...
      case 0x43484152: // CHAR; ASCII character
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::TEXT, 1, 0, 0));
        break;
      case 0x544E414D: // TNAM; type name
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::INTEGER, Format::TEXT, 4, 0, 0));
        break;
      case 0x52454354: // RECT; QuickDraw Rectangle
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::RECT, Format::DECIMAL, 2, 0, 0));
        break;
      case 0x434F4C52: // COLR; QuickDraw Color RGB Triplet
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::COLOR, Format::HEX, 2, 0, 0, false));
        break;
      case 0x4C53545A: // LSTZ
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::LIST_ZERO_BYTE, Format::FLAG, 0, 0, 0));
        write_stack.emplace_back(&entries->back()->list_entries);
        break;
      case 0x4C535442: // LSTB
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::LIST_EOF, Format::FLAG, 0, 0, 0));
        write_stack.emplace_back(&entries->back()->list_entries);
        break;
      case 0x5A434E54: // ZCNT
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::LIST_ZERO_COUNT, Format::HEX, 2, 0, 0));
        write_stack.emplace_back(&entries->back()->list_entries);
        break;
      case 0x4F434E54: // OCNT
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::LIST_ONE_COUNT, Format::HEX, 2, 0, 0));
        write_stack.emplace_back(&entries->back()->list_entries);
        break;
      case 0x4C434E54: // LCNT; Resorcerer-only. One-based long count of list items
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::LIST_ONE_COUNT, Format::HEX, 4, 0, 0));
        write_stack.emplace_back(&entries->back()->list_entries);
        break;
      case 0x4C535443: { // LSTC
        // ZCNT or OCNT should have already opened the list; make sure that it was the previous command.
        if (write_stack.size() < 2) {
          throw std::runtime_error("LSTC without preceding ZCNT/OCNT/LCNT in TMPL");
        }
        if (!write_stack.back()->empty()) {
          throw std::runtime_error("commands between ZCNT/OCNT/LCNT and LSTC");
        }
        auto list_type = write_stack[write_stack.size() - 2]->back()->type;
        if ((list_type != Type::LIST_ZERO_COUNT) && (list_type != Type::LIST_ONE_COUNT)) {
          throw std::runtime_error("LSTC used for non-ZCNT/OCNT list");
        }
        break;
      }
      ...
      case 0x50313030: // P100; Pnnn = Pascal string with size 0xnnn / fixed-size Pascal string with NUL-byte padding
        ...
        //
        // This appears to be a bug. Stuffit Expander has a TMPL with a P100 field in it, but P100 isn't valid - a
        // 1-byte pstring can only be 0xFF bytes long. It looks like whoever wrote the TMPL mistakely included the
        // length byte here... reading some of the resources that would match the relevant TMPL makes it look like we
        // should treat P100 as if it were P0FF, so that's what we'll do.
        entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::FIXED_PSTRING, Format::TEXT, 0xFF, 0, 0));
        break;
      default:
        try {
          if ((type & 0xFF000000) == 0x48000000) {
            // Hnnn (fixed-length hex dump)
            uint16_t width = phosg::value_for_hex_char(type & 0xFF) |
                (phosg::value_for_hex_char((type >> 8) & 0xFF) << 4) |
                (phosg::value_for_hex_char((type >> 16) & 0xFF) << 8);
            entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::STRING, Format::HEX, width, 0, 0));
          } else if ((type & 0xFF000000) == 0x43000000) {
            // Cnnn (C string with fixed NUL-byte padding)
            uint16_t width = phosg::value_for_hex_char(type & 0xFF) |
                (phosg::value_for_hex_char((type >> 8) & 0xFF) << 4) |
                (phosg::value_for_hex_char((type >> 16) & 0xFF) << 8);
            entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::FIXED_CSTRING, Format::TEXT, width, 0, 0));
          } else if ((type & 0xFFFF0000) == 0x50300000) {
            // P0nn (Pascal string with fixed NUL-byte padding)
            uint16_t width = phosg::value_for_hex_char(type & 0xFF) |
                (phosg::value_for_hex_char((type >> 8) & 0xFF) << 4);
            entries->emplace_back(std::make_unique<Entry>(std::move(name), Type::FIXED_PSTRING, Format::TEXT, width, 0, 0));
          } else {
            throw std::runtime_error("unknown field type: " + string_for_resource_type(type));
          }
  ...
  if (write_stack.size() != 1) {
    throw std::runtime_error("unterminated list in TMPL");
  }
  return ret;
}

std::string ResourceFile::describe_template(const TemplateEntryList& tmpl) {
  using Entry = ResourceFile::TemplateEntry;
  using Type = Entry::Type;
  using Format = Entry::Format;

  std::deque<std::string> lines;
  std::function<void(const std::vector<std::unique_ptr<Entry>>& entries, size_t indent_level)> process_entries = [&](const std::vector<std::unique_ptr<Entry>>& entries, size_t indent_level) {
      ...
      switch (entry->type) {
        ...
        case Type::POINT_2D: {
          ...
          if (!entry->case_names.empty()) {
            ...
            case_names_str = " (" + phosg::join(tokens, ", ") + ")";
          }
        ...
        case Type::FIXED_PSTRING:
          // Note: The length byte is NOT included in entry->width, in contrast to FIXED_CSTRING (where the \0 at the
          // end IS included). This is why we +1 here.
        ...
        case Type::OPT_EOF:
          ...
          break;
        default:
          throw std::logic_error("unknown entry type in TMPL");
      }
    }
  };
...
static std::string format_template_string(
  ...
  if (format == Format::HEX) {
    ...
    throw std::logic_error("invalid string display format");
  }
};

static std::string format_template_integer(const std::unique_ptr<ResourceFile::TemplateEntry>& entry, int64_t value) {
  using Entry = ResourceFile::TemplateEntry;
  using Format = Entry::Format;

  std::string case_name_suffix;
  try {
  ...
  switch (entry->format) {
    ...
    case Format::DATE: {
      ...
      if (ts < 0) {
        ...
        return std::format("{} (classic: 0x{:X}){}",
            phosg::format_time(ts * 1000000), static_cast<uint64_t>(value), case_name_suffix);
      }
    }
    default:
      throw std::logic_error("invalid integer display format");
  }
};

static std::string format_template_bool(const std::unique_ptr<ResourceFile::TemplateEntry>& entry, bool value) {
  std::string case_name_suffix;
  try {
    case_name_suffix = std::format(" ({})", entry->case_names.at(value));
  } catch (const std::out_of_range&) {
...
static void format_list_item(
    std::deque<std::string>& lines,
    phosg::StringReader& r,
    const std::unique_ptr<ResourceFile::TemplateEntry>& entry,
    size_t indent_level,
    size_t z) {
  std::deque<std::string> temp_lines;
  disassemble_from_template_inner(temp_lines, r, entry->list_entries, indent_level + 1);

  std::string item_prefix(indent_level * 2, ' ');
  item_prefix += std::format("{}:", z);

  // When the inner template is a single line, prefix it with the array index; otherwise, put the array index on its
  // own line
  if (temp_lines.size() == 1) {
    std::string last = temp_lines.back();
    phosg::strip_leading_whitespace(last);
    lines.emplace_back(item_prefix + " " + last);
  } else {
    lines.emplace_back(item_prefix);
    for (const std::string& l : temp_lines) {
  ...
  for (const auto& entry : entries) {
    ...
    switch (entry->type) {
      ...
      case Type::INTEGER: {
        ...
        if (entry->is_signed) {
          if (entry->width == 1) {
            ...
            throw std::logic_error("invalid width in disassemble_from_template");
          }
        } else {
          if (entry->width == 1) {
            value = r.get_u8();
        ...
        if (entry->end_alignment) {
          throw std::logic_error("integer has nonzero end_alignment");
        }
      ...
      case Type::ALIGNMENT: {
        ...
        if (boundary != 0) {
          // We currently only support offset == 0 or (offset == 1 and boundary == 2) since those seem to be the only
          // cases supported by the TMPL format.
          if (offset == 1) {
            if (boundary != 2) {
              throw std::logic_error("boundary must be 2 when offset is 1");
            }
            if (!(r.where() & 1)) {
              r.skip(1);
            }
            ...
            throw std::logic_error("offset is not 1 or 0");
          }
        }
        break;
      }
      case Type::FIXED_POINT: {
        ...
        if (entry->format == Format::DECIMAL) {
          ...
          throw std::logic_error("invalid fixed-point display format");
        }
        break;
      }
      case Type::EOF_STRING:
      ...
      case Type::CSTRING: {
        ...
        if (entry->end_alignment == 2) {
          ...
          throw std::logic_error("unsupported pstring end alignment");
        }
        break;
      }
      case Type::FIXED_PSTRING: {
        ...
        if (size > entry->width) {
          throw std::runtime_error("p-string too long for field");
        }
      ...
      case Type::POINT_2D: {
        ...
        std::string x_str = format_template_integer(entry, pt.x);
        std::string y_str = format_template_integer(entry, pt.y);
        lines.emplace_back(prefix + "x=" + x_str + ", y=" + y_str);
        break;
      }
      case Type::RECT: {
        Rect rect = r.get<Rect>();
        std::string x1_str = format_template_integer(entry, rect.x1);
        std::string y1_str = format_template_integer(entry, rect.y1);
        std::string x2_str = format_template_integer(entry, rect.x2);
        std::string y2_str = format_template_integer(entry, rect.y2);
      ...
      case Type::LIST_ONE_COUNT: {
        ...
        if (entry->width == 2) {
          ...
          // It's (currently) not possible to get a LIST_ZERO_COUNT with a 4-byte width field
          if (entry->type == Type::LIST_ZERO_COUNT) {
            throw std::logic_error("4-byte width LIST_ZERO_COUNT");
          }
          num_items = r.get_u32b();
        } else {
          throw std::logic_error("invalid list length width");
        }
      ...
      default:
        throw std::logic_error("unknown field type in disassemble_from_template");
    }
  }
}

std::string ResourceFile::disassemble_from_template(
    const void* data, size_t size, const ResourceFile::TemplateEntryList& tmpl) {
  phosg::StringReader r(data, size);
  std::deque<std::string> lines;
...
ResourceFile::DecodedSizeResource ResourceFile::decode_SIZE(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedVersionResource ResourceFile::decode_vers(std::shared_ptr<const Resource> res) {
...
std::vector<ResourceFile::DecodedCodeFragmentEntry> ResourceFile::decode_cfrg(int16_t id, uint32_t type) const {
  return this->decode_cfrg(this->get_resource(type, id));
}

std::vector<ResourceFile::DecodedCodeFragmentEntry> ResourceFile::decode_cfrg(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_cfrg(res->data.data(), res->data.size());
}

std::vector<ResourceFile::DecodedCodeFragmentEntry> ResourceFile::decode_cfrg(const void* vdata, size_t size) {
  phosg::StringReader r(vdata, size);

  const auto& header = r.get<CodeFragmentResourceHeader>();
  if (header.version != 1) {
    throw std::runtime_error("cfrg is not version 1");
  }

  std::vector<DecodedCodeFragmentEntry> ret;
  while (ret.size() < header.entry_count) {
    ...
    if (src_entry.usage > 4) {
      throw std::runtime_error("code fragment entry usage is invalid");
    }
    ret_entry.usage = static_cast<DecodedCodeFragmentEntry::Usage>(src_entry.usage);

    if (src_entry.usage > 4) {
      throw std::runtime_error("code fragment entry location (where) is invalid");
    }
    ...
    ret_entry.name = std::string(&src_entry.name[1], static_cast<size_t>(src_entry.name[0]));
    ret_entry.extension_count = src_entry.extension_count;

    // TODO: it looks like there is probably some alignment logic that we should implement here (see System cfrg 0, for
    // example)
    size_t extension_data_end_offset = entry_start_offset + src_entry.entry_size;
    if (r.where() > extension_data_end_offset) {
      throw std::runtime_error("code fragment entry size is smaller than header + name");
    }
...
std::vector<uint32_t> parse_relocation_data(phosg::StringReader& r) {
  // Note: we intentionally do not check r.eof here, since the format has an explicit end command
  std::vector<uint32_t> ret;
  uint32_t offset = 0;
  for (;;) {
    uint8_t a = r.get_u8();
    if (a == 0) {
      ...
      if (a == 0) {
        ...
        throw std::runtime_error("invalid relocation command (0001-007F)");
      }
    } else if (a & 0x80) {
...
ResourceFile::DecodedCode0Resource ResourceFile::decode_CODE_0(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_CODE_0(res->data.data(), res->data.size());
}

ResourceFile::DecodedCode0Resource ResourceFile::decode_CODE_0(const void* vdata, size_t size) {
  phosg::StringReader r(vdata, size);
  ...
  // a multiple of 8, we can't just check r.eof() here.
...
ResourceFile::DecodedDriverResource ResourceFile::decode_DRVR(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_DRVR(res->data.data(), res->data.size());
}

ResourceFile::DecodedDriverResource ResourceFile::decode_DRVR(const void* data, size_t size) {
  phosg::StringReader r(data, size);
  ...
  auto handle_label = [&](uint16_t* dest, uint16_t src, const char* name) {
    if ((src != 0) && (src < ret.code_start_offset)) {
      throw std::runtime_error(std::string(name) + " label is before code start");
    } else {
      *dest = src;
    }
  };
...
ResourceFile::DecodedRSSCResource ResourceFile::decode_RSSC(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_RSSC(res->data.data(), res->data.size());
}

ResourceFile::DecodedRSSCResource ResourceFile::decode_RSSC(const void* data, size_t size) {
  phosg::StringReader r(data, size);

  const auto& header = r.get<RSSCResourceHeader>();
  if (header.type_signature != RESOURCE_TYPE_RSSC) {
    throw std::runtime_error("incorrect type signature");
  }
  ...
  for (size_t z = 0; z < function_count; z++) {
    if ((header.functions[z] != 0) && (header.functions[z] < sizeof(header))) {
      throw std::runtime_error("function offset points within header");
    } else {
...
ResourceFile::DecodedDecompressorResource ResourceFile::decode_dcmp(std::shared_ptr<const Resource> res) {
...
PEFFile ResourceFile::decode_pef(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedPEFDriver ResourceFile::decode_expt(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedPEFDriver ResourceFile::decode_nsrd(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedColorIconResource ResourceFile::decode_cicn(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedColorIconResource ResourceFile::decode_cicn(const void* vdata, size_t size) {
  ...
  phosg::ImageGA11 bitmap_img;
  if (header.bitmap_header.flags_row_bytes) {
    bitmap_img = phosg::ImageGA11(header.bitmap_header.bounds.width(), header.bitmap_header.bounds.height());
...
ResourceFile::DecodedColorCursorResource ResourceFile::decode_crsr(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedColorCursorResource ResourceFile::decode_crsr(const void* vdata, size_t size) {
  ...
  if ((header.type & 0xFFFE) != 0x8000) {
    throw std::runtime_error("unknown crsr type");
  }
...
static ResourceFile::DecodedPattern decode_ppat_data(phosg::StringReader& r) {
  const auto& header = r.get<PixelPatternResourceHeader>();

  phosg::Image monochrome_pattern = decode_monochrome_image(&header.monochrome_pattern, 8, 8, 8);

  // Type 0 is monochrome; type 1 is indexed color; type 2 is apparently RGB color, but it's not clear if these are
  // ever stored in resources or only used when loaded in memory
  // TODO: Handle type 2 properly if it is indeed RGB
  if ((header.type == 0) || (header.type == 2)) {
    phosg::Image color_pattern = monochrome_pattern.convert_monochrome_to_color();
    return {std::move(color_pattern), std::move(monochrome_pattern), header.monochrome_pattern};
  }
  if ((header.type != 1) && (header.type != 3)) {
    throw std::runtime_error("unknown ppat type");
  }
  ...
  // We can't know the color table size until we have the struct, so pget it again with the correct size to check that
  // it's all within bounds
  r.pget<ColorTable>(pixmap_header.color_table_offset, ctable.size());

  // Decode the color image
  phosg::Image pattern = decode_color_image(pixmap_header, pixmap_data, &ctable);
...
ResourceFile::DecodedPattern ResourceFile::decode_ppat(std::shared_ptr<const Resource> res) {
...
std::vector<ResourceFile::DecodedPattern> ResourceFile::decode_pptN(int16_t id, uint32_t type) const {
  return this->decode_pptN(this->get_resource(type, id));
}

std::vector<ResourceFile::DecodedPattern> ResourceFile::decode_pptN(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_pptN(res->data.data(), res->data.size());
}

std::vector<ResourceFile::DecodedPattern> ResourceFile::decode_pptN(const void* vdata, size_t size) {
  // These resources are composed of a 2-byte count field, then N 4-byte offsets, then the ppat data
  ...
  std::vector<DecodedPattern> ret;
  for (size_t x = 0; x < count; x++) {
    phosg::StringReader sub_r = r.subx(offsets[x], offsets[x + 1] - offsets[x]);
    ret.emplace_back(decode_ppat_data(sub_r));
  }
  return ret;
}

phosg::ImageG1 ResourceFile::decode_PAT(int16_t id, uint32_t type) const {
  return this->decode_PAT(this->get_resource(type, id));
}

phosg::ImageG1 ResourceFile::decode_PAT(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_PAT(res->data.data(), res->data.size());
}

phosg::ImageG1 ResourceFile::decode_PAT(const void* data, size_t size) {
  if (size != 8) {
    throw std::runtime_error("PAT not exactly 8 bytes in size");
  }
  return decode_monochrome_image(data, size, 8, 8);
}

std::vector<phosg::ImageG1> ResourceFile::decode_PATN(int16_t id, uint32_t type) const {
  return this->decode_PATN(this->get_resource(type, id));
}

std::vector<phosg::ImageG1> ResourceFile::decode_PATN(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_PATN(res->data.data(), res->data.size());
}

std::vector<phosg::ImageG1> ResourceFile::decode_PATN(const void* vdata, size_t size) {
  phosg::StringReader r(vdata, size);
  uint16_t num_patterns = r.get_u16b();

  std::vector<phosg::ImageG1> ret;
...
std::vector<phosg::ImageG1> ResourceFile::decode_SICN(int16_t id, uint32_t type) const {
  return this->decode_SICN(this->get_resource(type, id));
}

std::vector<phosg::ImageG1> ResourceFile::decode_SICN(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_SICN(res->data.data(), res->data.size());
}

std::vector<phosg::ImageG1> ResourceFile::decode_SICN(const void* vdata, size_t size) {
  // So simple, there isn't even a header struct! SICN resources are just several 0x20-byte monochrome images
  // concatenated together.

  if (size & 0x1F) {
    throw std::runtime_error("SICN size not a multiple of 32");
  }
...
phosg::ImageRGBA8888N apply_mask_from_parallel_icon_list(FnT&& decode_list, const phosg::ImageRGB888& color_image) {
  try {
    auto decoded_mask = decode_list();
    if (decoded_mask.composite.empty()) {
      throw std::runtime_error("corresponding mask resource is not a 2-icon list");
    }
    return apply_alpha_from_mask(color_image, decoded_mask.composite);
  } catch (const std::exception&) {
    return color_image.change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>();
  }
}

phosg::ImageRGBA8888N ResourceFile::decode_ics8(int16_t id, uint32_t type) const {
  return this->decode_ics8(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_ics8(std::shared_ptr<const Resource> res) const {
...
phosg::ImageRGB888 ResourceFile::decode_ics8_without_alpha(const void* data, size_t size) {
  return decode_8bit_image(data, size, 16, 16);
}

phosg::ImageRGBA8888N ResourceFile::decode_kcs8(int16_t id, uint32_t type) const {
  return this->decode_kcs8(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_kcs8(std::shared_ptr<const Resource> res) const {
  return this->decode_ics8(res);
}

phosg::ImageRGB888 ResourceFile::decode_kcs8_without_alpha(const void* data, size_t size) {
  return ResourceFile::decode_ics8_without_alpha(data, size);
}

phosg::ImageRGBA8888N ResourceFile::decode_icl8(int16_t id, uint32_t type) const {
  return this->decode_icl8(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_icl8(std::shared_ptr<const Resource> res) const {
...
phosg::ImageRGB888 ResourceFile::decode_icl8_without_alpha(const void* data, size_t size) {
  return decode_8bit_image(data, size, 32, 32);
}

phosg::ImageRGBA8888N ResourceFile::decode_icm8(int16_t id, uint32_t type) const {
  return this->decode_icm8(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_icm8(std::shared_ptr<const Resource> res) const {
...
phosg::ImageRGB888 ResourceFile::decode_icm8_without_alpha(const void* data, size_t size) {
  return decode_8bit_image(data, size, 16, 12);
}

phosg::ImageRGBA8888N ResourceFile::decode_ics4(int16_t id, uint32_t type) const {
  return this->decode_ics4(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_ics4(std::shared_ptr<const Resource> res) const {
...
phosg::ImageRGB888 ResourceFile::decode_ics4_without_alpha(const void* data, size_t size) {
  return decode_4bit_image(data, size, 16, 16);
}

phosg::ImageRGBA8888N ResourceFile::decode_kcs4(int16_t id, uint32_t type) const {
  return this->decode_kcs4(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_kcs4(std::shared_ptr<const Resource> res) const {
  return this->decode_ics4(res);
}

phosg::ImageRGB888 ResourceFile::decode_kcs4_without_alpha(const void* data, size_t size) {
  return ResourceFile::decode_ics4_without_alpha(data, size);
}

phosg::ImageRGBA8888N ResourceFile::decode_icl4(int16_t id, uint32_t type) const {
  return this->decode_icl4(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_icl4(std::shared_ptr<const Resource> res) const {
  auto decoded = this->decode_icl4_without_alpha(res->data.data(), res->data.size());
  return apply_mask_from_parallel_icon_list(
      [&]() { return this->decode_ICNN(res->id, RESOURCE_TYPE_ICNN); }, std::move(decoded));
}

phosg::ImageRGB888 ResourceFile::decode_icl4_without_alpha(const void* data, size_t size) {
  return decode_4bit_image(data, size, 32, 32);
}

phosg::ImageRGBA8888N ResourceFile::decode_icm4(int16_t id, uint32_t type) const {
  return this->decode_icm4(this->get_resource(type, id));
}

phosg::ImageRGBA8888N ResourceFile::decode_icm4(std::shared_ptr<const Resource> res) const {
  auto decoded = this->decode_icm4_without_alpha(res->data.data(), res->data.size());
  return apply_mask_from_parallel_icon_list(
      [&]() { return this->decode_icmN(res->id, RESOURCE_TYPE_icmN); }, std::move(decoded));
}

phosg::ImageRGB888 ResourceFile::decode_icm4_without_alpha(const void* data, size_t size) {
  return decode_4bit_image(data, size, 16, 12);
}

phosg::ImageG1 ResourceFile::decode_ICON(int16_t id, uint32_t type) const {
  return this->decode_ICON(this->get_resource(type, id));
}

phosg::ImageG1 ResourceFile::decode_ICON(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_ICON(res->data.data(), res->data.size());
}

phosg::ImageG1 ResourceFile::decode_ICON(const void* data, size_t size) {
...
ResourceFile::DecodedCursorResource ResourceFile::decode_CURS(std::shared_ptr<const Resource> res) {
...
static ResourceFile::DecodedIconListResource decode_monochrome_image_list(
  ...
  if (w & 7) {
    throw std::logic_error("monochrome icons must be a multiple of 8 pixels wide");
  }

  size_t image_bytes = (w >> 3) * h;
  if (size % image_bytes) {
    throw std::runtime_error("data size is not a multiple of image_bytes");
  }
  size_t num_icons = size / image_bytes;

  if (num_icons == 2) {
    return {.composite = decode_monochrome_image_masked(data, size, w, h), .images = std::vector<phosg::ImageG1>()};
  } else {
    std::vector<phosg::ImageG1> ret;
    ...
    return {.composite = phosg::ImageGA11(), .images = std::move(ret)};
  }
}

ResourceFile::DecodedIconListResource ResourceFile::decode_ICNN(int16_t id, uint32_t type) const {
  return this->decode_ICNN(this->get_resource(type, id));
}

ResourceFile::DecodedIconListResource ResourceFile::decode_ICNN(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_ICNN(res->data.data(), res->data.size());
}

ResourceFile::DecodedIconListResource ResourceFile::decode_ICNN(const void* data, size_t size) {
...
ResourceFile::DecodedIconListResource ResourceFile::decode_icsN(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedIconListResource ResourceFile::decode_kcsN(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedIconListResource ResourceFile::decode_icmN(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedIconImagesResource::DecodedIconImagesResource() : icon_composer_version(0.0) {}
...
ResourceFile::DecodedIconImagesResource ResourceFile::decode_icns(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_icns(res->data.data(), res->data.size());
}

static phosg::ImageRGBA8888N decode_icns_packed_rgb_argb(
    const void* data, size_t size, size_t w, size_t h, bool has_alpha) {
  phosg::ImageRGBA8888N ret(w, h);

  // It appears that some applications support uncompressed data in these formats. It's not clear how the decoder
  // should determine whether the input is compressed or not... the logical heuristic (which we implement here) is that
  // if the data is smaller than it should be, then it's compressed. However, it appears that even in the
  // has_alpha=false case, the alpha field is still present in the uncompressed data, but is unused... so we compare
  // the size as if the image was ARGB even if it's not.
  size_t pixel_count = w * h;
  if (size >= (pixel_count * 4)) {
    // In the uncompressed case, the channels aren't tightly packed; instead, each pixel's values are stored next to
    // each other in (A|X)RGB order.
    phosg::StringReader r(data, size);
    for (size_t y = 0; y < h; y++) {
      for (size_t x = 0; x < w; x++) {
        ...
        ret.write(x, y, phosg::rgba8888(pixel_r, pixel_g, pixel_b, has_alpha ? pixel_a : 0xFF));
      }
    }

  } else {
    std::string decompressed_data = decompress_packed_icns_data(data, size);
    if (decompressed_data.size() < pixel_count * (has_alpha ? 4 : 3)) {
      throw std::runtime_error("not enough decompressed data");
    }

    phosg::StringReader ar(decompressed_data.data(), pixel_count);
    phosg::StringReader rr(decompressed_data.data() + pixel_count * (has_alpha ? 1 : 0), pixel_count);
    phosg::StringReader gr(decompressed_data.data() + pixel_count * (has_alpha ? 2 : 1), pixel_count);
    phosg::StringReader br(decompressed_data.data() + pixel_count * (has_alpha ? 3 : 2), pixel_count);
    for (size_t y = 0; y < h; y++) {
      for (size_t x = 0; x < w; x++) {
        ret.write(x, y, phosg::rgba8888(rr.get_u8(), gr.get_u8(), br.get_u8(), has_alpha ? ar.get_u8() : 0xFF));
      }
    }
  }
...
static void add_jpeg2000_png_or_direct_color(
  ...
  if (sec_size < 8) {
    throw std::runtime_error("JPEG2000/PNG/direct color section is too small");
  }
  // TODO: What is the "right" way to detect ARGB format? Here we just assume JPEG2000 or PNG if the respective image
  // formats' header bytes appear, which should work in essentially all cases.
  if (!memcmp(sec_data, "\0\0\0\x0CjP  ", 8) || !memcmp(sec_data, "\0\0\0\x0CjP2 ", 8)) {
    std::string data(reinterpret_cast<const char*>(sec_data), sec_size);
    ...
    phosg::print_data(stderr, sec_data, sec_size);
    throw std::runtime_error(
        "icns subfield is not PNG, JPEG2000, or packed direct color: " + string_for_resource_type(sec_type));
  }
}
...
ResourceFile::DecodedIconImagesResource ResourceFile::decode_icns(const void* data, size_t size) {
  ...
  if (r.get_u32b() != RESOURCE_TYPE_icns) {
    throw std::runtime_error("resource does not begin with icns tag");
  }
  if (r.get_u32b() > size) {
    throw std::runtime_error("resource size field is incorrect");
  }
      ...
      switch (sec_type) {
        ...
        case RESOURCE_TYPE_ICON: // 32x32 no alpha
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_ICON(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_icmN: { // 16x12 with mask
          auto decoded = ResourceFile::decode_icmN(sec_data, sec_size);
          if (decoded.composite.empty()) {
            throw std::runtime_error("icm# subfield is not a 2-icon list");
          }
          ret.type_to_image.emplace(sec_type, decoded.composite.change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        }
        case RESOURCE_TYPE_icsN: { // 16x16 with mask
          auto decoded = ResourceFile::decode_icsN(sec_data, sec_size);
          if (decoded.composite.empty()) {
            throw std::runtime_error("ics# subfield is not a 2-icon list");
          }
          ret.type_to_image.emplace(sec_type, decoded.composite.change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        }
        case RESOURCE_TYPE_ICNN: { // 32x32 with mask
          ...
          if (decoded.composite.empty()) {
            throw std::runtime_error("ICN# subfield is not a 2-icon list");
          }
          ret.type_to_image.emplace(sec_type, decoded.composite.change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        }
        ...
        case RESOURCE_TYPE_icm4: // 16x12
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_icm4_without_alpha(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_ics4: // 16x16
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_ics4_without_alpha(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_icl4: // 32x32
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_icl4_without_alpha(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_ich4: // 48x48
          ret.type_to_image.emplace(sec_type,
              decode_4bit_image(sec_data, sec_size, 48, 48).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;

        // Fixed-size 8-bit images
        case RESOURCE_TYPE_icm8: // 16x12
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_icm8_without_alpha(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_ics8: // 16x16
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_ics8_without_alpha(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_icl8: // 32x32
          ret.type_to_image.emplace(sec_type,
              ResourceFile::decode_icl8_without_alpha(sec_data, sec_size).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_ich8: // 48x48
          ret.type_to_image.emplace(sec_type,
              decode_8bit_image(sec_data, sec_size, 48, 48).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;

        // Fixed-size 8-bit alpha-only images
        case RESOURCE_TYPE_s8mk:
          ret.type_to_image.emplace(sec_type,
              decode_8bit_image(sec_data, sec_size, 16, 16, nullptr).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_l8mk:
          ret.type_to_image.emplace(sec_type,
              decode_8bit_image(sec_data, sec_size, 32, 32, nullptr).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_h8mk:
          ret.type_to_image.emplace(sec_type,
              decode_8bit_image(sec_data, sec_size, 48, 48, nullptr).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        case RESOURCE_TYPE_t8mk:
          ret.type_to_image.emplace(sec_type,
              decode_8bit_image(sec_data, sec_size, 128, 128, nullptr).change_pixel_format<phosg::PixelFormat::RGBA8888_NATIVE>());
          break;
        ...
        case RESOURCE_TYPE_it32:
          ...
          if (sec_size < 4) {
            throw std::runtime_error("it32 data is too small");
          }
        ...
        case RESOURCE_TYPE_sbtp:
          ret.template_icns = std::make_shared<DecodedIconImagesResource>(ResourceFile::decode_icns(sec_data, sec_size));
          break;
        case RESOURCE_TYPE_slct:
          ret.selected_icns = std::make_shared<DecodedIconImagesResource>(ResourceFile::decode_icns(sec_data, sec_size));
          break;
        case 0xFDD92FA8: // Why did they not use ASCII chars for this type?
          ret.dark_icns = std::make_shared<DecodedIconImagesResource>(ResourceFile::decode_icns(sec_data, sec_size));
          break;
        default:
          phosg::fwrite_fmt(stderr, "Warning: unknown section type in icns: {}\n", string_for_resource_type(sec_type));
          break;
      }
    } catch (const std::exception& e) {
      std::string type_str = string_for_resource_type(sec_type);
      throw std::runtime_error(std::format("within icns/{} (reader at {:X}): {}",
          type_str, r.where(), e.what()));
    }
  }

  // Generate composite images for the types that have masks
  auto find_mask = +[](const DecodedIconImagesResource& icns, uint32_t sec_type) -> const phosg::ImageRGBA8888N& {
    auto it = icns.type_to_image.find(sec_type);
    if (it == icns.type_to_image.end()) {
      throw std::out_of_range("no mask for image");
    }
    return it->second;
  };
  for (const auto& it : ret.type_to_image) {
    try {
      switch (it.first) {
        // For the classic types, the alpha is copied from the corresponding list image (ic*#)
        ...
        // For 32-bit types, the mask image is decoded to a non-alpha Image with the value in all three channels, so we
...
public:
  QuickDrawResourceDasmPort(const ResourceFile* rf, size_t x, size_t y)
    ...
    if (x >= 0x10000 || y >= 0x10000) {
      throw std::runtime_error("PICT resources cannot specify images larger than 65535x65535");
    }
  }
  ...
  phosg::ImageRGBA8888N& image() {
    if (this->img.empty()) {
      // Initialize with transparent so undrawn areas blend with background, matching classic Mac DrawPicture behavior
      this->img = phosg::ImageRGBA8888N(this->bounds.width(), this->bounds.height(), 0x00000000);
    }
    return this->img;
  }
  const phosg::ImageRGBA8888N& image() const {
  ...
  virtual void blit(
    ...
    if (mask.get()) {
      ...
      if (effective_mask_rect.x1 != dest_x ||
        ...
        std::string mask_rect_str = mask->rect.str();
        std::string effective_mask_rect_str = effective_mask_rect.str();
        throw std::runtime_error(std::format(
            "mask region rect {} with effective {} is not same as dest rect [{}, {}, {}, {}]",
            mask_rect_str, effective_mask_rect_str, dest_x, dest_y, dest_x + w, dest_y + h));
      }
  ...
  virtual void set_bounds(Rect z) {
    // In rare cases the bounds are incorrect for the image size, and a drawing opcode can actually resize them. This
    // happens with SMC-encoded images, for example. So, we have to be able to handle resizing the canvas after port
    // creation.
  ...
  phosg::ImageRGB888 pen_pixel_pattern;
  virtual const phosg::ImageRGB888& get_pen_pixel_pattern() const {
    return this->pen_pixel_pattern;
  }
  virtual void set_pen_pixel_pattern(phosg::ImageRGB888&& z) {
    this->pen_pixel_pattern = std::move(z);
  }

  phosg::ImageRGB888 fill_pixel_pattern;
  virtual const phosg::ImageRGB888& get_fill_pixel_pattern() const {
    return this->fill_pixel_pattern;
  }
  virtual void set_fill_pixel_pattern(phosg::ImageRGB888&& z) {
    this->fill_pixel_pattern = std::move(z);
  }

  phosg::ImageRGB888 background_pixel_pattern;
  virtual const phosg::ImageRGB888& get_background_pixel_pattern() const {
    return this->background_pixel_pattern;
  }
  virtual void set_background_pixel_pattern(phosg::ImageRGB888&& z) {
...
ResourceFile::DecodedPICTResource ResourceFile::decode_PICT_data(
  ...
  try {
    ...
    try {
      ...
      return {phosg::ImageRGBA8888N(), e.extension, e.data};
    }
    ...
    phosg::Subprocess proc({"picttoppm", "-noheader"}, -1, -1, fileno(stderr));
    std::string ppm_data = proc.communicate(data, size, 10000000);
    int proc_ret = proc.wait(true);
    if (proc_ret != 0) {
      throw std::runtime_error(std::format("picttoppm failed ({})", proc_ret));
    }
    if (ppm_data.empty()) {
      throw std::runtime_error("picttoppm succeeded but produced no output");
    }
    return {phosg::ImageRGBA8888N::from_file_data(ppm_data), "", ""};
#else
    throw;
#endif
  }
...
ResourceFile::DecodedSoundResource ResourceFile::decode_snd_data(
  ...
  if (size < 4) {
    throw std::runtime_error("snd doesn\'t even contain a format code");
  }
  ...
  if (format_code32 == 0x43756523 || format_code32 == 0x44617461) {
    ...
    while (r.remaining() >= sizeof(SoundResourceHeaderMohawkChunkHeader)) {
      ...
      if (header.type == 0x43756523) {
        ...
        // TODO: we should obviously support different values for these fields but I currently don't have any example
        // files with different values so I can't tell how the samples are interleaved, or even if num_samples is
        // actually num_bytes, num_frames, or something else.
        if (data_header.num_channels != 1) {
          throw std::runtime_error("MHK snd does not have exactly 1 channel");
        }
    ...
    throw std::runtime_error("MHK snd does not contain a Data section");
  }
  ...
  if (format_code16 == 0x0001) {
    ...
    if (header.data_format_count == 0) {
      ...
      if (data_format.data_format_id != 5) {
        throw std::runtime_error("snd data format is not sampled");
      }
      ret.num_channels = (data_format.flags & 0x40) ? 2 : 1;

    } else {
      throw std::runtime_error("snd has multiple data formats");
    }
    ...
    if ((header.type & 0xFFFFFF00) != 0x6D706700) {
      throw std::runtime_error("format 3 snd is not mp3");
    }

    // TODO: for little-endian samples, do we just byteswap the entire stream?
    if (header.is_little_endian) {
      throw std::runtime_error("format 3 snd is little-endian");
    }
    // TODO: for encrypted samples, do we just call decrypt_soundmusicsys_data on the sample buffer?
    if (header.is_encrypted) {
      throw std::runtime_error("format 3 snd is encrypted");
    }
    if (decompress_ysnd) {
    ...
    throw std::runtime_error("snd is not format 1 or 2");
  }

  if (num_commands == 0) {
    throw std::runtime_error("snd contains no commands");
  }
  ...
  for (size_t x = 0; x < num_commands; x++) {
    ...
    static const std::unordered_map<uint16_t, const char*> command_names({
        {0x0003, "quiet"},
        {0x0004, "flush"},
        {0x0005, "reinit"},
        {0x000A, "wait"},
    ...
    switch (command.command) {
      ...
      case 0x8051: // play sampled sound
        if (sample_buffer_offset) {
          throw std::runtime_error("snd contains multiple buffer commands");
        }
        sample_buffer_offset = command.param2;
        break;
      default:
        try {
          throw std::runtime_error(std::format("command not implemented: {:04X} ({}) {:04X} {:08X}",
  ...
  // Some snds have an incorrect sample buffer offset, but they still play! I guess Sound Manager ignores the offset in
  // the command? (We do so here)
  ...
  if (sample_buffer.encoding == 0x00) {
    if (sample_buffer.data_bytes == 0) {
      throw std::runtime_error("snd contains no samples");
    }
    ...
    // Hack: it appears Beatnik archives set the stereo flag even when the snd is mono, so we ignore it in that case.
    // TODO: Does this also apply to MACE3/6? I'm assuming it does here, but haven't verified this. Also, what about
    ...
    switch (compressed_buffer.compression_id) {
      case 0xFFFE:
        throw std::runtime_error("snd uses variable-ratio compression");
      ...
      default:
        throw std::runtime_error("snd is compressed using unknown algorithm");
    }

  } else {
    throw std::runtime_error(std::format("unknown encoding for snd data: {:02X}", sample_buffer.encoding));
  }
}
...
ResourceFile::DecodedSoundResource ResourceFile::decode_snd(std::shared_ptr<const Resource> res, bool metadata_only) const {
...
ResourceFile::DecodedSoundResource ResourceFile::decode_csnd(
    std::shared_ptr<const Resource> res, bool metadata_only) const {
  return ResourceFile::decode_csnd(res->data.data(), res->data.size(), metadata_only);
}

ResourceFile::DecodedSoundResource ResourceFile::decode_csnd(const void* data, size_t size, bool metadata_only) const {
  phosg::StringReader r(data, size);
  uint32_t type_and_size = r.get_u32b();

  uint8_t sample_type = type_and_size >> 24;
  if ((sample_type > 3) && (sample_type != 0xFF)) {
    throw std::runtime_error("invalid csnd sample type");
  }

  // Check that decompressed_size makes sense for the type (for types 1 and 2, it must be a multiple of 2; for type 3,
  ...
  if (sample_type != 0xFF) {
    ...
    if (decompressed_size % sample_bytes) {
      throw std::runtime_error("decompressed size is not a multiple of frame size");
    }
  }
  ...
  std::string decompressed = decompress_soundmusicsys_lzss(r.getv(compressed_size), compressed_size);
  if (decompressed.size() < decompressed_size) {
    throw std::runtime_error("decompression did not produce enough data");
  }
  ...
  if (sample_type == 0) { // mono8
    ...
    phosg::be_uint16_t* data = reinterpret_cast<phosg::be_uint16_t*>(decompressed.data());
    phosg::be_uint16_t* data_end = data + decompressed.size();
...
ResourceFile::DecodedSoundResource ResourceFile::decode_esnd(int16_t id, uint32_t type, bool metadata_only) const {
...
ResourceFile::DecodedSoundResource ResourceFile::decode_esnd(const void* data, size_t size, bool metadata_only) const {
  std::string decrypted = decrypt_soundmusicsys_data(data, size);
...
ResourceFile::DecodedSoundResource ResourceFile::decode_ESnd(
  ...
  std::string data(reinterpret_cast<const char*>(vdata), size);
...
ResourceFile::DecodedSoundResource ResourceFile::decode_Ysnd(int16_t id, uint32_t type, bool metadata_only) const {
...
std::string ResourceFile::decode_cmid(int16_t id, uint32_t type) const {
  return this->decode_cmid(this->get_resource(type, id));
}

std::string ResourceFile::decode_cmid(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_cmid(res->data.data(), res->data.size());
}

std::string ResourceFile::decode_cmid(const void* data, size_t size) {
  return decompress_soundmusicsys_data(data, size);
}

std::string ResourceFile::decode_emid(int16_t id, uint32_t type) const {
  return this->decode_emid(this->get_resource(type, id));
}

std::string ResourceFile::decode_emid(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_emid(res->data.data(), res->data.size());
}

std::string ResourceFile::decode_emid(const void* vdata, size_t size) {
  return decrypt_soundmusicsys_data(vdata, size);
}

std::string ResourceFile::decode_ecmi(int16_t id, uint32_t type) const {
  return this->decode_ecmi(this->get_resource(type, id));
}

std::string ResourceFile::decode_ecmi(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_ecmi(res->data.data(), res->data.size());
}

std::string ResourceFile::decode_ecmi(const void* data, size_t size) {
  std::string decrypted = decrypt_soundmusicsys_data(data, size);
...
ResourceFile::DecodedInstrumentResource ResourceFile::decode_INST(std::shared_ptr<const Resource> res) const {
...
ResourceFile::DecodedInstrumentResource ResourceFile::decode_INST_recursive(
  ...
  if (!ids_in_progress.emplace(res->id).second) {
    throw std::runtime_error("reference cycle between INST resources");
  }

  phosg::StringReader r(res->data);
  ...
  // If the UseSampleRate flag is not set, then the synthesizer apparently doesn't correct for sample rate differences
  // at all. This means that if your INSTs refer to snds that are 11025kHz but you're playing at 22050kHz, your song
  // will be shifted up an octave. Even worse, if you have snds with different sample rates, the pitches of all notes
  // will be messed up. (It would seem like this should be enabled in all cases, but apparently it's not enabled in a
  // lot of cases, and some songs depend on that. Sigh...)
  ...
  auto add_key_region = [&](int16_t snd_id, uint8_t key_low, uint8_t key_high) {
    ...
    if (snd_type == RESOURCE_TYPE_INST) {
      // TODO: Should we apply overrides from the current INST here? (Should we take into account header.base_note, for
      // example?) I seem to recall seeing some sort of nominal abstraction within SoundMusicSys that would support
      // e.g. applications of base_notes from both INSTs in succession, but I'm too lazy to go find it right now. For
      // now, we just copy the relevant parts of the other INST's key regions into this one; this seems to give the
      // correct result for the INSTs with subordinates that I've seen.
      ...
      for (const auto& sub_rgn : sub_inst.key_regions) {
        ...
        // Clamp the sub region's low/high keys to the requested range and copy it into the current INST
        ret.key_regions.emplace_back(
            std::max<uint8_t>(key_low, sub_rgn.key_low),
            std::min<uint8_t>(key_high, sub_rgn.key_high),
            sub_rgn.base_note,
            sub_rgn.snd_id,
            sub_rgn.snd_type);
      }
      ...
      // If the snd has PlayAtSampledFreq, set a fake base_note of 0x3C to ignore whatever the snd/csnd/esnd says.
...
static ResourceFile::DecodedSongResource decode_SONG_SMS(const void* vdata, size_t size) {
  phosg::StringReader r(vdata, size);

  const auto& header = r.get<SMSSongResourceHeader>();

  // Note: They split the pitch shift field in a later version of the library; some older SONGs that have a negative
  // value in the pitch_shift field may also set filter_type to 0xFF because it was part of pitch_shift before. We
  // currently don't use filter_type at all, so we don't bother changing it.
...
static ResourceFile::DecodedSongResource decode_SONG_RMF(const void* vdata, size_t size) {
  phosg::StringReader r(vdata, size);
  ...
  for (uint16_t x = 0; x < header.num_subresources; x++) {
    ...
    if (type == 0x524D4150) { // RMAP (instrument override)
      ...
      throw std::runtime_error("unknown SONG subresource type");
    }
  }
...
ResourceFile::DecodedSongResource ResourceFile::decode_SONG(std::shared_ptr<const Resource> res) const {
...
std::string ResourceFile::decode_Tune(int16_t id, uint32_t type) const {
  return this->decode_Tune(this->get_resource(type, id));
}

std::string ResourceFile::decode_Tune(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_Tune(res->data.data(), res->data.size());
}

std::string ResourceFile::decode_Tune(const void* vdata, size_t size) {
  ...
  while (!r.eof()) {
    ...
    switch (type) {
      ...
      case 0x0F: { // Metadata message
        ...
        std::string message_data = r.readx(message_size - 4);
        if (message_data.size() != message_size - 4) {
          throw std::runtime_error("metadata message exceeds track boundary");
        }

        // The second-to-last word is the message type
        uint16_t message_type = *reinterpret_cast<const phosg::be_uint16_t*>(message_data.data() + message_data.size() - 4) & 0x3FFF;
        ...
        switch (message_type) {
          case 1: { // Instrument definition
            if (message_size != 0x5C) {
              throw std::runtime_error("message size is incorrect");
            }
            uint32_t instrument = *reinterpret_cast<const phosg::be_uint32_t*>(message_data.data() + 0x50);
          ...
          case 6: { // Extended (?) instrument definition
            ...
            uint32_t instrument = *reinterpret_cast<const phosg::be_uint32_t*>(message_data.data() + 0x7C);
            events.emplace_back(current_time, 0xC0 | channel, instrument);
            events.emplace_back(current_time, 0xB0 | channel, 7, 0x7F); // Volume
            events.emplace_back(current_time, 0xB0 | channel, 10, 0x40); // Panning
            events.emplace_back(current_time, 0xE0 | channel, 0x00, 0x40); // Pitch bend
          ...
          default:
            throw std::runtime_error(std::format("unknown metadata event {:08X}/{:X} (end offset 0x{:X})",
      ...
      default:
        throw std::runtime_error("unsupported event in stream");
    }
  }
...
static const std::string mac_roman_table_rtf[0x100] = {
    // clang-format off
    // 00
    ...
    // 10
    "\\'10", "\xE2\x8C\x98", "\xE2\x87\xA7", "\xE2\x8C\xA5", "\xE2\x8C\x83", "\\'15", "\\'16", "\\'17",
...
ResourceFile::DecodedStringSequence ResourceFile::decode_STRN(std::shared_ptr<const Resource> res) {
...
std::string ResourceFile::decode_STRN_entry(std::shared_ptr<const Resource> res, size_t index) {
  return ResourceFile::decode_STRN_entry(res->data.data(), res->data.size(), index);
}

std::string ResourceFile::decode_STRN_entry(const void* vdata, size_t size, size_t index) {
  phosg::StringReader r(vdata, size);
  size_t count = r.get_u16b();

  if (index >= count) {
    throw std::out_of_range("STR# index out of range");
  }
...
std::vector<std::string> ResourceFile::decode_TwCS(int16_t id, uint32_t type) const {
  return this->decode_TwCS(this->get_resource(type, id));
}

std::vector<std::string> ResourceFile::decode_TwCS(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_TwCS(res->data.data(), res->data.size());
}

std::vector<std::string> ResourceFile::decode_TwCS(const void* data, size_t size) {
  phosg::StringReader r(data, size);
  size_t count = r.get_u16b();
  std::vector<std::string> ret;
  while (ret.size() < count) {
    ...
    if (!is_encrypted) {
      ...
      std::string& decrypted = ret.emplace_back();
...
ResourceFile::DecodedString ResourceFile::decode_STR(std::shared_ptr<const Resource> res) {
...
std::string ResourceFile::decode_card(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_card(res->data.data(), res->data.size());
}

std::string ResourceFile::decode_card(const void* vdata, size_t size) {
  if (size == 0) {
    return "";
  }
  phosg::StringReader r(vdata, size);
...
std::string ResourceFile::decode_TEXT(int16_t id, uint32_t type) const {
  return this->decode_TEXT(this->get_resource(type, id));
}

std::string ResourceFile::decode_TEXT(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_TEXT(res->data.data(), res->data.size());
}

std::string ResourceFile::decode_TEXT(const void* data, size_t size) {
  return decode_mac_roman(reinterpret_cast<const char*>(data), size);
}

std::string ResourceFile::decode_styl(int16_t id, uint32_t type) const {
  return this->decode_styl(this->get_resource(type, id));
}

std::string ResourceFile::decode_styl(std::shared_ptr<const Resource> res) const {
  // Get the text now, so we'll fail early if there's no resource
  std::string text;
  try {
    text = this->get_resource(RESOURCE_TYPE_TEXT, res->id)->data;
  } catch (const std::out_of_range&) {
    throw std::runtime_error("style has no corresponding TEXT");
  }

  if (text.empty()) {
    throw std::runtime_error("corresponding TEXT resource is empty");
  }
  ...
  std::string ret = "{\\rtf1\\ansi\n{\\fonttbl";

  // Collect all the fonts and write the font table
  std::map<uint16_t, uint16_t> font_table;
  for (size_t x = 0; x < num_commands; x++) {
  ...
  std::map<uint64_t, uint16_t> color_table;
  ...
  for (size_t x = 0; x < num_commands; x++) {
    ...
    if (offset >= text.size()) {
      throw std::runtime_error("offset is past end of TEXT resource data");
    }
    if (end_offset <= offset) {
      throw std::runtime_error("block size is zero or negative");
    }
    std::string text_block = text.substr(offset, end_offset - offset);

    // TODO: We can produce smaller files by omitting commands for parts of the format that haven't changed
...
ResourceFile::DecodedKeyCharMap ResourceFile::decode_KCHR(const void* data, size_t size) {
  ...
  if (version != 0) {
    throw std::runtime_error(std::format("unknown KCHR version: {}", version));
  }
...
ResourceFile::DecodedFontResource ResourceFile::decode_FONT(std::shared_ptr<const Resource> res) const {
...
ResourceFile::DecodedFontResource ResourceFile::decode_FONT_only(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedFontResource ResourceFile::decode_NFNT(std::shared_ptr<const Resource> res) const {
...
ResourceFile::DecodedFontResource ResourceFile::decode_NFNT_only(std::shared_ptr<const Resource> res) {
...
ResourceFile::DecodedFontResource ResourceFile::decode_FONT_data(
  ...
  if (ret.source_bit_depth == 1) {
    ...
    throw std::runtime_error("2-bit font bitmaps are not implemented");
  } else if (ret.source_bit_depth == 4) {
    throw std::runtime_error("4-bit font bitmaps are not implemented");
  } else if (ret.source_bit_depth == 8) {
    throw std::runtime_error("8-bit font bitmaps are not implemented");
  } else {
    throw std::logic_error("unknown font bit depth");
  }
  auto glyphs_image = ret.full_bitmap.convert_monochrome_to_color<phosg::PixelFormat::RGB888>();

  // +2 here because last_char is inclusive, and there's the missing glyph at the end as well
  ...
  for (uint32_t ch = header.first_char; ch < header.first_char + num_glyphs; ch++) {
    ...
    if (glyph.offset < 0) {
      throw std::runtime_error("glyphs with negative offsets are not implemented");
    }

    if (glyph.width > 0) {
      glyph.img = phosg::ImageGA11(glyph.width + glyph.offset, header.rect_height);
...
std::vector<ResourceFile::DecodedFontInfo> ResourceFile::decode_finf(int16_t id, uint32_t type) const {
  return this->decode_finf(this->get_resource(type, id));
}

std::vector<ResourceFile::DecodedFontInfo> ResourceFile::decode_finf(std::shared_ptr<const Resource> res) {
  return ResourceFile::decode_finf(res->data.data(), res->data.size());
}

std::vector<ResourceFile::DecodedFontInfo> ResourceFile::decode_finf(const void* data, size_t size) {
  if (size == 0) {
    return {};
  }
  ...
  std::vector<DecodedFontInfo> ret;
  for (size_t x = 0; x < count; x++) {
...
ResourceFile::DecodedROMOverridesResource ResourceFile::decode_ROvN(std::shared_ptr<const Resource> res) {
...
std::vector<ResourceFile::DecodedDialogItem> ResourceFile::decode_DITL(const void* data, size_t size) {
  ...
  std::vector<DecodedDialogItem> ret;
  while (ret.size() < num_items) {
    ...
    if (info_is_res_id) {
      phosg::StringReader r(item.text);
      item.resource_id = r.get_s16b();
      if (!r.eof()) {
        throw std::runtime_error("incorrect info string length");
      }
      item.text.clear();
    }
