...
static constexpr struct {
  uint32_t icns_type;
  // The type's size in Icon Archiver archive. Identical to type's size in .icns file if and only if the type is not 24
  // bit RGB
  uint32_t size_in_archive;
  // 24 bit RGB type instead of indexed, b/w or alpha?
  bool is_24_bits;
  // The bit in the bitfield of an icon that indicates which icon types exist (the bits aren't in the same order as the
  // icon data)
  uint8_t type_bit;
} ICON_TYPES[] = {
    // These are in the order the icon types are stored in an Icon Archiver 4 file's icon data
    {ResourceDASM::RESOURCE_TYPE_ICNN, 256, false, 5},
    {ResourceDASM::RESOURCE_TYPE_icl4, 512, false, 6},
    {ResourceDASM::RESOURCE_TYPE_icl8, 1024, false, 7},
    {ResourceDASM::RESOURCE_TYPE_il32, 4096, true, 8},
    {ResourceDASM::RESOURCE_TYPE_l8mk, 1024, false, 9},

    {ResourceDASM::RESOURCE_TYPE_icsN, 64, false, 0},
    {ResourceDASM::RESOURCE_TYPE_ics4, 128, false, 1},
    {ResourceDASM::RESOURCE_TYPE_ics8, 256, false, 2},
    {ResourceDASM::RESOURCE_TYPE_is32, 1024, true, 3},
    {ResourceDASM::RESOURCE_TYPE_s8mk, 256, false, 4},

    {ResourceDASM::RESOURCE_TYPE_ichN, 576, false, 10},
    {ResourceDASM::RESOURCE_TYPE_ich4, 1152, false, 11},
    {ResourceDASM::RESOURCE_TYPE_ich8, 2304, false, 12},
    {ResourceDASM::RESOURCE_TYPE_ih32, 9216, true, 13},
    {ResourceDASM::RESOURCE_TYPE_h8mk, 2304, false, 14},
};
...
static uint8_t icns_type_to_icns_idx(uint32_t icns_type) {
  ...
  throw std::logic_error("Unsupported icns icon type");
}

// Order must match the one in `ICON_TYPES` above, use helper function to guarantee this
static const uint8_t ICON_TYPE_ICNN = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ICNN);
static const uint8_t ICON_TYPE_icl4 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_icl4);
static const uint8_t ICON_TYPE_icl8 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_icl8);
static const uint8_t ICON_TYPE_il32 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_il32);
static const uint8_t ICON_TYPE_l8mk = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_l8mk);

static const uint8_t ICON_TYPE_icsN = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_icsN);
static const uint8_t ICON_TYPE_ics4 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ics4);
static const uint8_t ICON_TYPE_ics8 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ics8);
static const uint8_t ICON_TYPE_is32 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_is32);
static const uint8_t ICON_TYPE_s8mk = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_s8mk);

static const uint8_t ICON_TYPE_ichN = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ichN);
static const uint8_t ICON_TYPE_ich4 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ich4);
static const uint8_t ICON_TYPE_ich8 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ich8);
static const uint8_t ICON_TYPE_ih32 = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_ih32);
static const uint8_t ICON_TYPE_h8mk = icns_type_to_icns_idx(ResourceDASM::RESOURCE_TYPE_h8mk);

// .icns files must contain the icons in a specific order, namely b/w icons last, or they don't show up correctly in
// Finder
...
static void write_icns(
    ...
    uint32_t icon_number, const std::string& icon_name,
  ...
  std::string filename = std::format("{}/{}_{}", context.out_dir, context.base_name, icon_number);
  if (!icon_name.empty()) {
    filename += "_";
    // TODO: sanitize name
    filename += icon_name;
  ...
  for (unsigned int t = 0; t < ICON_TYPE_COUNT; ++t) {
    ...
    if (uncompressed_offsets[type] >= 0) {
      ...
      if (ICON_TYPES[type].is_24_bits) {
        // Icon Archiver stores 24 bit icons as ARGB. The .icns format requires them to be compressed one channel after
        // the other with a PackBits-like algorithm
        size = ResourceDASM::compress_strided_icns_data(
                   data, uncompressed_data + uncompressed_offsets[type] + 1, ICON_TYPES[type].size_in_archive, 4) +
            ResourceDASM::compress_strided_icns_data(
                data, uncompressed_data + uncompressed_offsets[type] + 2, ICON_TYPES[type].size_in_archive, 4) +
            ResourceDASM::compress_strided_icns_data(
                data, uncompressed_data + uncompressed_offsets[type] + 3, ICON_TYPES[type].size_in_archive, 4);
      } else {
      ...
      // If b/w icons are missing, write a black square as icon, and all pixels set as mask: color icons don't display
      // correctly without b/w icon+mask(?)
      data.put_u32b(ICON_TYPES[type].icns_type);
...
static void dearchive_icon(DearchiverContext& context, uint16_t version, uint32_t icon_number) {
  ...
  // Is the icon selected in Icon Archiver? (doesn't seem to be actually used by application)
  r.get_u16b();
  ...
  std::string uncompressed_data(uncompressed_icon_size, '\0');
  int32_t uncompressed_offsets[ICON_TYPE_COUNT];
  memset(uncompressed_offsets, -1, sizeof(uncompressed_offsets));

  std::string icon_name;
  if (version > 1) {
    // Version 2 has a bitfield of 15 bits (3 sizes, 5 color depths including mask) for each icon that specifies which
    // types of an icon family there are
    ...
    for (uint32_t type = 0; type < ICON_TYPE_COUNT; ++type) {
      ...
      if (offset > uncompressed_icon_size) {
        phosg::fwrite_fmt(stderr, "Warning: buffer overflow while decoding icon {}: {} > {}. Skipping...\n", icon_number, offset, uncompressed_icon_size);
        r.go(r_where + icon_size);
        return;
      }
    }
    if (offset == 0) {
      phosg::fwrite_fmt(stderr, "Warning: icon {} contains no supported icon types. Skipping...\n", icon_number);
      r.go(r_where + icon_size);
      return;
    }
    ...
    if (zlib_result != 0) {
      phosg::fwrite_fmt(stderr, "Warning: zlib error decompressing icon {}: {}\n. Skipping...", icon_number, zlib_result);
      r.go(r_where + icon_size);
      return;
    }
    if (uncompressed_size_zlib != uncompressed_icon_size) {
      phosg::fwrite_fmt(stderr, "Warning: decompressed icon {} is of size {} instead of {} as expected\n. Skipping...", icon_number, uncompressed_size_zlib, uncompressed_icon_size);
      r.go(r_where + icon_size);
      return;
    }
  } else {
    // Version 1 uses an array of offsets from a position before the icon's name. Before System 8.5 there were only 6
    // icon types:
    ...
    //   ics4    16x16x4
    //   ics8    16x16x8
    // An offset of 0 means that the icon type doesn't exist. The offsets aren't  always in ascending order. They are
    // into the *uncompressed* data.
    ...
    // The offsets don't start at 0, i.e. they aren't relative to the beginning of the compressed icon data, but
    // relative to somewhere before the icon's name
    uint16_t offset_base = icon_name.size() + 17;

    // All icons are compressed as a single blob with PackBits
    ResourceDASM::unpack_bits_into(r, uncompressed_data.data(), uncompressed_icon_size);
  ...
  phosg::strip_trailing_whitespace(icon_name);
  icon_name = ResourceDASM::decode_mac_roman(icon_name, /*for filename=*/true);

  write_icns(context, icon_number, icon_name, uncompressed_data.data(), uncompressed_offsets);

  // Done: continue right after the icon, skipping any possible padding after the icon's data
  r.go(r_where + icon_size);
}
...
int main(int argc, const char** argv) {
  try {
    ...
    std::string content = phosg::load_file(context.base_name);
    phosg::StringReader& r = context.in = phosg::StringReader(content);

    // Check signature ('QBSE' 'PACK')
    if (r.get_u32b() != 0x51425345 || r.get_u32b() != 0x5041434B) {
      phosg::fwrite_fmt(stderr, "File '{}' isn't an Icon Archiver file\n", context.base_name);
      return 2;
    }
    ...
    if (version != 1 && version != 2) {
      phosg::fwrite_fmt(stderr, "File '{}' has unsupported version {}\n", context.base_name, version);
      return 2;
    }
    ...
    if (version > 1) {
      ...
      if (r.get_u32b() != 0x49415542) {
        phosg::fwrite_fmt(stderr, "File '{}' isn't an Icon Archiver version 2 file\n", context.base_name);
        return 2;
      }
      ...
      r.get_u8();
      auto copyright = ResourceDASM::decode_mac_roman(r.readx(63));
      phosg::strip_trailing_zeroes(copyright);

      r.get_u8();
      auto comment = ResourceDASM::decode_mac_roman(r.readx(255));
      phosg::strip_trailing_zeroes(comment);
