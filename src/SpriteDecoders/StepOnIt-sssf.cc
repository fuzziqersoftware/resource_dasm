...
phosg::ImageRGBA8888N decode_sssf_image(phosg::StringReader& r, const std::vector<ColorTableEntry>& clut) {
  ...
  phosg::StringReader data_r = r.sub(data_stream_offset, r.size() - data_stream_offset);
  ...
  while (decoded_data.size() < target_size) {
    ...
    for (size_t x = 0; x < num_zeroes; x++) {
      if (decoded_data.size() >= target_size) {
        throw std::logic_error("exceeded target size during transparent segment");
      }
    ...
    for (size_t x = 0; x < num_data_bytes; x++) {
      if (decoded_data.size() >= target_size) {
        throw std::logic_error("exceeded target size during data segment");
      }
  ...
  phosg::StringReader decoded_r(decoded_data.data(), decoded_data.size());
...
std::vector<phosg::ImageRGBA8888N> decode_sssf(const std::string& data, const std::vector<ColorTableEntry>& clut) {
  ...
  std::map<uint32_t, ssize_t> offsets;
  ...
  for (auto it = offsets.begin(); it != offsets.end(); it++) {
    ...
    phosg::StringReader sub_r = r.sub(it->first, end_offset - it->first);
