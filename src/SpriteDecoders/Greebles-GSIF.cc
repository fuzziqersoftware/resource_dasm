...
phosg::ImageRGB888 decode_GSIF(const std::string& gsif_data, const std::vector<ColorTableEntry>& pltt) {
  ...
  if (header.magic != 0x47534946) {
    throw std::runtime_error("incorrect GSIF signature");
  }

  phosg::ImageRGB888 ret(header.width, header.height);
  auto write = [&](size_t x, size_t y, uint8_t index) {
    if (!pltt.empty()) {
      ret.write(x, y, pltt.at(index).c.rgba8888());
    } else {
      ret.write(x, y, phosg::rgba8888_gray(index));
    }
  };
  ...
  for (size_t y = 0; y < header.height; y++) {
    ...
    while ((x < header.width) && (r.where() < expected_end_offset)) {
      ...
      if (cmd < 0x40) {
        ...
        // 40-5F: (c-3F) 8-byte 2-color blocks, with bitmask denoting which color to use for each pixel. A 0 in the
        // bitmask means to use the first color.
        ...
        // 60-7E: (c-5D) 4-byte 4-color blocks, with indexes in extra bytes. A 00 in the index field means to use the
        // first color.
        ...
        size_t count = (cmd < 0xFB) ? (cmd - 0x7D) : ((((cmd - 0xFB) << 8) | r.get_u8()) + 0x7E);
    ...
    if (x != header.width) {
      throw std::runtime_error("row did not produce enough data");
    }
    if (r.where() != expected_end_offset) {
      throw std::runtime_error("row ended at incorrect offset");
    }
  }
  return ret;
}
