...
std::string decompress_SHAP_lz(const std::string& data) {
  ...
  // TODO: The original code allocates 0x442 bytes. Are those extra 0x42 bytes important? Seems like the code never
  // uses them.
  uint8_t dict[0x400];
  memset(dict, 0, sizeof(dict));
  uint16_t dict_offset = 0x3BE;

  // control_bits holds 8 bits from the LZSS control stream at a time. The low bits are the actual control bits; the
  // high 8 bits specify which of the low bits are still available for use. When the high bits are all 0, a new control
  // byte must be read.
...
std::string decompress_SHAP_rows_rle(const std::string& data, size_t num_rows, size_t row_bytes) {
  ...
  for (size_t x = 0; x < num_rows; x++) {
    ...
    phosg::StringReader row_r = r.sub(r.where(), bytes);
    r.skip(bytes);
    ...
    if (size_after_row - size_before_row != row_bytes) {
      throw std::runtime_error("incorrect result row length");
    }
  }
