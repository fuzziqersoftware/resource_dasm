...
ImageG1 decode_1img(const string& data) {
  return decode_monochrome_image(data.data(), data.size(), 32, 21);
}

ImageRGB888 decode_4img(const string& data, const vector<ColorTableEntry>& clut) {
  auto clut8 = to_color8(clut);
  return decode_4bit_image(data.data(), data.size(), 32, 21, &clut8);
}

ImageRGB888 decode_8img(const string& data, const vector<ColorTableEntry>& clut) {
  auto clut8 = to_color8(clut);
