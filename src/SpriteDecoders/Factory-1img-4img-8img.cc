...
// These are all just fixed-size, fixed-depth images. Oddly, their dimensions don't really make sense - the game uses
// 20x20-pixel icons, but the height of all formats is 21 pixels (and the last row is blank). The monochrome width
// being 32 internally makes sense, as on 68K systems row_bytes would have to be at least 4; the other formats wouldn't
