...
std::string decompress_PPic_pixel_map_data(const std::string& data, size_t row_bytes, size_t height) {
  ...
  phosg::StringWriter current_rows[4];
      ...
      switch (cmd & 0xF0) {
        ...
        case 0x30:
          ...
          for (size_t line = 0; line < 4; line++) {
            ...
            if (row.size() < 2) {
              throw std::runtime_error("repeat command given before any blocks were written");
            }
        ...
        // 4X YZ [...] - Write (X + 1) 2-color blocks. Each block is given by a phosg::be_uint16_t following YZ, where
        //   the first 4 bits specify the colors in row 0 (0=Y, 1=Z), the next 4 specify row 1, etc.
        ...
        // 6X ABCD [...] - Write (X + 1) 4-color blocks. Each block is given by a phosg::be_uint32_t following ABCD,
        //   where the first 8 bits specify the colors in row 0 (2 bits for each pixel; 0=A, 1=B, 2=C, 3=D), etc.
        ...
        case 0x90:
          ...
          throw std::runtime_error("no-op command in stream");
...
std::string decompress_PPic_bitmap_data(const std::string& data, size_t row_bytes, size_t height) {
  ...
  if (w.str().size() != row_bytes * height) {
    throw std::runtime_error("decompression produced the wrong amount of data");
  }
  ...
  while (ret.size() < count) {
    ...
    if (r.get_u16b(false) & 0x8000) { // Color (pixel map)
      ...
      if (header.color_table_offset == 0xFFFFFFFF) {
        if (clut.empty()) {
          throw std::runtime_error("PPic does not have embedded color table, and no clut was provided");
        }
        ...
        if (header.color_table_offset != 0) {
          throw std::runtime_error("PPic embedded color table does not immediately follow header");
        }
