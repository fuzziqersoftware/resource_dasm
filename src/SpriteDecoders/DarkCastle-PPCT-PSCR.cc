...
std::string decompress_PSCR_v1(phosg::StringReader& r) {
  ...
  while (!r.eof()) {
    ...
    if (cmd == 0) {
      ...
      // Note: cmd is 01-7F here, so it's always safe to subtract 1. Also, looks like the last byte in the long const
      // table never gets used...
...
std::string decompress_PPCT(phosg::StringReader& r, size_t expected_bits) {
  if (expected_bits & 7) {
    throw std::runtime_error("expected bits is not a multiple of 8");
  }
  ...
  if (expected_bits != 0) {
    if (w.size() > expected_bits) {
      // Some sprites overflow the boundaries of the output buffer by a few bits. A few of them overflow by a lot of
      // bits (80 or more), but the images appear correct, so... I guess it's OK to just always ignore the extra output
      w.truncate(expected_bits);
    } else {
      // Similarly, some sprites can end early if their lower-right corners are white. Just extend the result to the
      // required length.
...
struct PPCTHeader {
  // The type field is always 0-9, but there are really only two types: 0, 3, or 9; or the others (1, 2, 4, 5, 6, 7,
  // 8). The code treats all types within those two groups as identical to each other.
  phosg::be_uint16_t type;
  phosg::be_uint16_t num_images;
  phosg::be_uint16_t width_words;
  phosg::be_uint16_t image_height_pixels;
  phosg::be_uint16_t unknown3;
  phosg::be_uint16_t unknown4;
  phosg::be_uint16_t unknown5;
  // Some useful values aren't contains in the header but can be easily computed from its values:
...
phosg::ImageGA11 decode_PPCT(const std::string& data) {
  ...
  if (type == 5) {
    ...
    throw std::runtime_error("type == 5");
  }
  std::string decompressed_data = use_ppct_v2 ? decompress_PSCR_v2(r) : decompress_PPCT(r, width * height);
  phosg::ImageG1 decoded = decode_monochrome_image(decompressed_data.data(), decompressed_data.size(), width, height);
  if (has_masks) {
    phosg::ImageGA11 ret(decoded.get_width(), h.num_images * h.image_height_pixels);
    ...
    return decoded.change_pixel_format<phosg::PixelFormat::GA11>();
  }
}
