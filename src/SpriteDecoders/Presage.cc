...
static phosg::ImageRGBA8888N decode_PPSS_lzss_section(phosg::StringReader& r, size_t w, size_t h, const std::vector<ColorTableEntry>& clut) {
  ...
  std::string decompressed = decompress_presage_lzss(compressed_data, compressed_bytes, max_output_bytes);
...
phosg::ImageGA11 decode_presage_mono_image(phosg::StringReader& r, size_t width, size_t height, bool use_and_compositing) {
  ...
  // leftmost 16 pixels in the top row. The next two words specify the values for the next 16 pixels in the top row,
  // and so on.
  width = (width + 15) & (~15);
  phosg::ImageGA11 ret(width, height);
      ...
      for (size_t z = 0; z < 16; z++) {
        if (use_and_compositing) {
          if (color_bits & 0x8000) {
            ret.write(x + z, y, 0x000000FF);
          } else {
            ret.write(x + z, y, (mask_bits & 0x8000) ? 0x00000000 : 0xFFFFFFFF);
          }
        } else {
          if (mask_bits & 0x8000) {
            ret.write(x + z, y, 0x00000000);
          } else {
            ret.write(x + z, y, (color_bits & 0x8000) ? 0x000000FF : 0xFFFFFFFF);
          }
        }
        mask_bits <<= 1;
        color_bits <<= 1;
  ...
  while (!should_stop) {
    ...
    switch (cmd & 0x60) {
      case 0x00:
        ...
        // (It makes sense for them to include the stop opcode here - to write a single byte, the command R0100000
        // could be used instead.) Note that we incremented C by 1 earlier for convenience, so we check for 1 rather
        // than 0 here.
        if (count == 1) {
          should_stop = true;
        } else {
      ...
      case 0x60:
        ...
        // If C == 0, go back to previous loop point if there are still iterations to run. If C != 0, push the current
        // location on the stack, along with the count. The commands from here through the corresponding R1100000
        // command will run (C + 1) times. (For example, if C == 1, we'll push (1, r.where()), then the intermediate
        // commands will run, then the R1100000 command at the end will see the count as 1 and will decrement it and
        // jump back. When it gets to the end command again, it will see (0, r.where()); it will then remove it and not
        // jump back.)
        count--;
        if (count != 0) {
  ...
  for (size_t z = 0; z < num_images; z++) {
    ...
    if (start_offset != 0) {
      ...
      if (format == 0x8002) {
        ...
        throw std::runtime_error("unknown Pak format");
      }
    }
  }
