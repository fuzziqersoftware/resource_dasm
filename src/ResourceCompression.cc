...
struct DecompressorImplementation {
  ...
  typedef std::string (*decompress_fn)(const CompressedResourceHeader& header, const void* source, size_t size);
  ...
  DecompressorImplementation(decompress_fn fn) : decompress(fn), data(nullptr), size(0), is_ppc(false) {}
  DecompressorImplementation(const void* data, size_t size, bool is_ppc)
      : decompress(nullptr), data(data), size(size), is_ppc(is_ppc) {}
};

static std::vector<DecompressorImplementation> get_candidate_decompressors(
  ...
  std::vector<DecompressorImplementation> ret;
...
struct M68KDecompressorInputHeader {
  // This structure speciies what the dcmp code expects to see on its stack at call time.

  // This is used to tell the program where to return to (stack pointer points here at entry time). On actual hardware,
  // this would be generated by a call opcode; in our case, emulation begins within the dcmp resource already, so we
  // simulate a call by adding this field before the actual parameters.
  phosg::be_uint32_t return_addr;

  // Parameters to the decompressor - the m68k calling convention passes args on the stack, so these are the actual
  // args to the function. The format is chosen based on the version field in the compressed resource's header.
  union {
    struct { // used when header_version == 8
      phosg::be_uint32_t data_size;
      phosg::be_uint32_t working_buffer_addr;
      phosg::be_uint32_t dest_buffer_addr;
      phosg::be_uint32_t source_buffer_addr;
    } __attribute__((packed)) v8;
    struct { // used when header_version == 9
      phosg::be_uint32_t source_resource_header;
      phosg::be_uint32_t dest_buffer_addr;
      phosg::be_uint32_t source_buffer_addr;
      phosg::be_uint32_t data_size;
  ...
  // This isn't part of the input parameters; this is just a convenient place to put a reset opcode (which stops
  // emulation cleanly) that the program can "return" to.
  phosg::be_uint16_t reset_opcode;
  phosg::be_uint16_t unused;
} __attribute__((packed));

struct PPC32DecompressorInputHeader {
  // As above, this is what an ncmp function expects on its stack at call time. Unlike in 68K-land, the PPC calling
  // convention passes the first several arguments in registers, so the data pointers, sizes, etc. aren't in this
  // structure at all.
  phosg::be_uint32_t saved_r1;
  phosg::be_uint32_t saved_cr;
  phosg::be_uint32_t saved_lr;
  phosg::be_uint32_t reserved1;
  phosg::be_uint32_t reserved2;
  phosg::be_uint32_t saved_r2;

  phosg::be_uint32_t unused[2];

  // This is where the program returns to; we set r2 to -1 (which should never happen normally) and make the syscall
  // handler stop emulation
  phosg::be_uint32_t set_r2_opcode;
  phosg::be_uint32_t syscall_opcode;
} __attribute__((packed));

std::shared_ptr<Resource> decompress_resource(
    std::shared_ptr<const Resource> res, uint64_t decompress_flags, const ResourceFile* context_rf) {
  if (res->data.size() < sizeof(CompressedResourceHeader)) {
    throw std::runtime_error("resource marked as compressed but is too small");
  }

  auto result = std::make_shared<ResourceFile::Resource>();
  result->type = res->type;
  result->id = res->id;
  result->flags = res->flags;
  result->name = res->name;

  const auto& header = *reinterpret_cast<const CompressedResourceHeader*>(res->data.data());
  if (header.magic != 0xA89F6572) {
    // It looks like some resources have the compression bit set but aren't actually compressed. Reverse-engineering
    // ResEdit makes it look like the Resource Manager just treats the resource as uncompressed if this value is
    // missing, so let's also not fail in that case.
  ...
  if (!(header.attributes & 0x01)) {
    throw std::runtime_error("resource marked as compressed but does not have compression attribute set");
  }
  ...
  if (header.header_version == 9) {
    ...
    throw std::runtime_error("compressed resource header version is not 8 or 9");
  }

  auto decompressors = get_candidate_decompressors(context_rf, dcmp_resource_id, decompress_flags);
  if (decompressors.empty()) {
    throw std::runtime_error("no decompressors are available for this resource");
  }

  if (verbose) {
    phosg::fwrite_fmt(stderr, "using dcmp/ncmp {} ({} implementation(s) available)\n",
        dcmp_resource_id, decompressors.size());
    phosg::fwrite_fmt(stderr, "note: data size is {} (0x{:X}); decompressed data size is {} (0x{:X}) bytes\n",
        res->data.size(), res->data.size(), header.decompressed_size, header.decompressed_size);
  }

  for (size_t z = 0; z < decompressors.size(); z++) {
    const auto& decompressor = decompressors[z];
    if (verbose) {
      phosg::fwrite_fmt(stderr, "attempting decompression with implementation {} of {}\n", z + 1, decompressors.size());
    }
    ...
    try {
      if (decompressor.decompress != nullptr) {
        ...
        std::string decompressed_data = decompressor.decompress(
            header,
        ...
        if (verbose) {
          float duration = static_cast<float>(phosg::now() - start_time) / 1000000.0f;
          phosg::fwrite_fmt(stderr,
              "note: decompressed resource using internal decompressor in {:g} seconds ({} -> {} bytes)\n",
        ...
        if (!decompressor.is_ppc) {
          ...
          // Figure out where in the dcmp to start execution. There appear to be two formats: one that has 'dcmp' in
          // bytes 4-8 where execution appears to just start at byte 0 (usually it's a branch opcode), and one where
          // the first three words appear to be offsets to various functions, followed by code. The second word appears
          // to be the main entry point in this format, so we use that to determine where to start execution.
          // TODO: It looks like the decompression implementation in ResEdit assumes the second format (with the three
          // offsets) if and only if the compressed resource has header format 9. This feels kind of bad because...
          // shouldn't the dcmp format be a property of the dcmp resource, not the resource being decompressed? We use
          // a heuristic here instead, which seems correct for all decompressors I've seen.
          uint32_t entry_offset;
          if (decompressor.size < 10) {
            throw std::runtime_error("decompressor resource is too short");
          }
          uint32_t internal_signature = *reinterpret_cast<const phosg::be_uint32_t*>(
              reinterpret_cast<const uint8_t*>(decompressor.data) + 4);
          if (internal_signature == RESOURCE_TYPE_dcmp) {
            entry_offset = 0;
          } else {
            // TODO: Call init and exit for decompressors that have them. It's not clear (yet) what the arguments to
            // init and exit should be... they each apparently take one argument based on how they adjust the stack
            // before returning, but every decompressor I've seen ignores the argument's value.
            entry_offset = *reinterpret_cast<const phosg::be_uint16_t*>(
                reinterpret_cast<const uint8_t*>(decompressor.data) + 2);
          }

          // Load the dcmp into emulated memory. dcmp resources are just raw 68K code; there's no header beyond what's
          // described above.
          ...
          if (verbose) {
            phosg::fwrite_fmt(stderr, "loaded code at {:08X}:{:X}\n", code_addr, code_region_size);
            phosg::fwrite_fmt(stderr, "dcmp entry offset is {:08X} (loaded at {:X})\n", entry_offset, entry_pc);
          }

        } else { // decompressor.is_ppc == true
          // ncmp resources are entire PEF files, so we have to parse the header and run relocations (if any) while
          // loading them.
          PEFFile f("<ncmp>", decompressor.data, decompressor.size);
          f.load_into("<ncmp>", mem, 0xF0000000);
          use_ppc_emulator = f.is_ppc();

          // ncmp decompressors don't appear to define any of the standard export symbols (init/main/term); instead,
          // they define a single export symbol in the export table.
          // TODO: It's possible that ncmps are allowed to define init and term. Presumably this would be similar to
          // how the unused functions work in dcmp v9 above... reverse-engineer ResEdit some more and figure this out.
          if (!f.init().name.empty()) {
            throw std::runtime_error("ncmp decompressor has init symbol");
          }
          if (!f.main().name.empty()) {
            throw std::runtime_error("ncmp decompressor has main symbol");
          }
          if (!f.term().name.empty()) {
            throw std::runtime_error("ncmp decompressor has term symbol");
          }
          const auto& exports = f.exports();
          if (exports.size() != 1) {
            throw std::runtime_error("ncmp decompressor does not export exactly one symbol");
          }

          // The start symbol is actually a transition vector, which is the code address followed by the desired value
          // in r2.
          std::string start_symbol_name = "<ncmp>:" + exports.begin()->second.name;
          ...
          if (verbose) {
            phosg::fwrite_fmt(stderr, "ncmp entry pc is {:08X} with r2 = {:08X}\n", entry_pc, entry_r2);
          }
        }
        ...
        // TODO: Looks like some decompressors expect zero bytes after the compressed input? Find out if this is true
        // and fix it if not.
        size_t input_region_size = res->data.size() + 0x100;
        // TODO: This is likely too big; should we use ((data.size() * 256) / working_buffer_fractional_size) instead?
        size_t working_buffer_region_size = res->data.size() * 256;

        // Set up data memory regions. Slightly awkward assumption: decompressed data is never more than 256 times the
        // size of the input data. We intentionally put the regions pretty far from each other in the address space in
        // order to fail catastrophically in case of buffer underflows or overflows; this is useful for debugging the
        // emulators.
        uint32_t stack_addr = 0x10000000;
        mem->allocate_at(stack_addr, stack_region_size);
        if (!stack_addr) {
          throw std::runtime_error("cannot allocate stack region");
        }
        uint32_t output_addr = 0x20000000;
        mem->allocate_at(output_addr, output_region_size);
        if (!output_addr) {
          throw std::runtime_error("cannot allocate output region");
        }
        ...
        if (!working_buffer_addr) {
          throw std::runtime_error("cannot allocate working buffer region");
        }
        uint32_t input_addr = 0xC0000000;
        mem->allocate_at(input_addr, input_region_size);
        if (!input_addr) {
          throw std::runtime_error("cannot allocate input region");
        }
        if (verbose) {
          phosg::fwrite_fmt(stderr, "memory:\n");
          phosg::fwrite_fmt(stderr, "  stack region at {:08X}:{:X}\n", stack_addr, stack_region_size);
          phosg::fwrite_fmt(stderr, "  output region at {:08X}:{:X}\n", output_addr, output_region_size);
          phosg::fwrite_fmt(stderr, "  working region at {:08X}:{:X}\n", working_buffer_addr, working_buffer_region_size);
          phosg::fwrite_fmt(stderr, "  input region at {:08X}:{:X}\n", input_addr, input_region_size);
        }
        ...
        if (use_ppc_emulator) {
          ...
          uint32_t return_addr = stack_addr + stack_region_size - sizeof(PPC32DecompressorInputHeader) +
              offsetof(PPC32DecompressorInputHeader, set_r2_opcode);
          ...
          // Create emulator
          auto interrupt_manager = std::make_shared<InterruptManager>();
          PPC32Emulator emu(mem);
          emu.set_interrupt_manager(interrupt_manager);

          // Set up registers. r3-r6 are the function arguments, which are analogous to the arguments to dcmp resources
          ...
          regs.pc = entry_pc;
          if (verbose) {
            phosg::fwrite_fmt(stderr, "initial stack contents (input header data):\n");
            phosg::print_data(stderr, input_header, sizeof(*input_header), regs.r[1].u);
          }

          // Set up the debugger, if debugging is enabled
          std::shared_ptr<EmulatorDebugger<PPC32Emulator>> debugger;
          if (trace_execution || debug_execution) {
            debugger = std::make_shared<EmulatorDebugger<PPC32Emulator>>();
            debugger->bind(emu);
          ...
          emu.set_syscall_handler([&](PPC32Emulator& emu) -> void {
            auto& regs = emu.registers();
            // We don't support any syscalls in PPC mode - the only syscall that should occur is the one at the end of
            // emulation, when r2 == -1.
          ...
          execution_start_time = phosg::now();
          try {
            emu.execute();
          } catch (const std::exception& e) {
            if (verbose) {
              uint64_t diff = phosg::now() - execution_start_time;
              float duration = static_cast<float>(diff) / 1000000.0f;
              phosg::fwrite_fmt(stderr, "powerpc decompressor execution failed ({:g}sec): {}\n", duration, e.what());
            }
            throw;
          }
          ...
          if (verbose) {
            phosg::fwrite_fmt(stderr, "initial stack contents (input header data):\n");
            phosg::print_data(stderr, input_header, sizeof(*input_header), regs.a[7]);
          }

          // Set up debugger
          std::shared_ptr<EmulatorDebugger<M68KEmulator>> debugger;
          if (trace_execution || debug_execution) {
            debugger = std::make_shared<EmulatorDebugger<M68KEmulator>>();
            debugger->bind(emu);
            debugger->state.mode = debug_execution ? DebuggerMode::STEP : DebuggerMode::TRACE;
          }
          ...
          emu.set_syscall_handler([&](M68KEmulator& emu, uint16_t opcode) -> void {
            ...
            // - Ben Mickaelian's self-modifying decompressor uses GetTrapAddress, but it suffices to simulate the
            //   asked-for traps with stubs since the dcmp doesn't seem to use the return value for anything important
            ...
            if (trap_number == 0x002E) { // BlockMove
              ...
              if (trap_number & 0x0800) {
                phosg::fwrite_fmt(stderr, "warning: skipping unimplemented toolbox trap (num={:X}, auto_pop={})\n",
                    static_cast<uint16_t>(trap_number & 0x0BFF), auto_pop ? "true" : "false");
              } else {
                phosg::fwrite_fmt(stderr, "warning: skipping unimplemented os trap (num={:X}, flags={})\n",
          ...
          try {
            ...
            if (verbose) {
              uint64_t diff = phosg::now() - execution_start_time;
              float duration = static_cast<float>(diff) / 1000000.0f;
              phosg::fwrite_fmt(stderr, "m68k decompressor execution failed ({:g}sec): {}\n", duration, e.what());
        ...
        if (verbose) {
          uint64_t diff = phosg::now() - execution_start_time;
          float duration = static_cast<float>(diff) / 1000000.0f;
          phosg::fwrite_fmt(stderr, "note: decompressed resource in {:g} seconds ({} -> {} bytes)\n",
      ...
      // If we get here, the resource was decompressed and res->data was replaced with the decompressed data
      result->flags = (res->flags & ~ResourceFlag::FLAG_COMPRESSED) | ResourceFlag::FLAG_DECOMPRESSED;
      return result;

    } catch (const std::exception& e) {
      if (verbose) {
        phosg::fwrite_fmt(stderr, "decompressor implementation {} of {} failed: {}\n", z + 1, decompressors.size(), e.what());
      }
    }
  }

  throw std::runtime_error("no decompressor succeeded");
}
