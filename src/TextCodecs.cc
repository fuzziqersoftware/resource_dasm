...
static constexpr const char mac_roman_table[0x100][4] = {
    // clang-format off
    // 00
    // Note: we intentionally incorrectly decode \r as \n here to convert CR line breaks to LF line breaks which modern
    // systems use
    "\x00", "\x01", "\x02", "\x03", "\x04", "\x05", "\x06", "\x07",
    "\x08", "\t", "\n", "\x0B", "\x0C", "\n", "\x0E",  "\x0F",
    // 10
    "\x10", "\xE2\x8C\x98", "\xE2\x87\xA7", "\xE2\x8C\xA5", "\xE2\x8C\x83", "\x15", "\x16", "\x17",
...
std::string decode_mac_roman(const char* data, size_t size, bool for_filename) {
  std::string ret;
  while (size--) {
...
std::string decode_mac_roman(char data, bool for_filename) {
...
std::string string_for_resource_type(uint32_t type, bool for_filename) {
  std::string result;
...
uint32_t resource_type_for_raw_string(const std::string& s) {
  switch (s.size()) {
    ...
    case 2:
      return ((static_cast<uint32_t>(s[0]) & 0xFF) << 24) | ((static_cast<uint32_t>(s[1]) & 0xFF) << 16) | 0x00002020;
    case 3:
