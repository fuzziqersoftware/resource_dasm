...
std::string SH4Emulator::disassemble_one(DisassemblyState& s, uint16_t op) {
  switch (op_get_op(op)) {
      ...
      switch (op_get_r3(op)) {
        case 0x2: {
          ...
          if (reg2 < reg_names.size()) {
            ...
            return string_printf("stc     r%hhu, r%hhub", reg1, reg2 & 7);
          }
          break;
        }
        case 0x3:
      ...
      switch (op_get_r3(op)) {
        ...
        case 0x3: {
          ...
          if (reg2 < reg_names.size()) {
            ...
            return string_printf("stc.l   -[r%hhu], r%hhub", op_get_r1(op), reg2 & 7);
          }
          break;
        }
        case 0x4:
        ...
        case 0x7: {
          ...
          if (reg2 < reg_names.size()) {
            ...
            return string_printf("ldc.l   r%hhub, [r%hhu]+", reg2 & 7, op_get_r1(op));
          }
          break;
        }
        case 0x8:
        ...
        case 0xE: {
          ...
          if (reg2 < reg_names.size()) {
            ...
            return string_printf("ldc     r%hhub, r%hhu", reg2 & 7, op_get_r1(op));
          }
          break;
        }
    ...
    case 0xC:
      switch (op_get_r1(op)) {
        ...
        case 0x7: // 11000111dddddddd mova   r0, [(pc & ~3) + 4 + disp * 4]
          return string_printf("mova    r0, [0x%08" PRIX32 "]", (s.pc & (~3)) + 4 + 4 * op_get_simm8(op));
    ...
    case 0xD: // 1101nnnndddddddd mov.l  rn, [(pc & ~3) + 4 + d * 4]
      return string_printf("mov.l   r%hhu, [0x%08" PRIX32 "]", op_get_r1(op), (s.pc & (~3)) + 4 + 4 * op_get_simm8(op));

    case 0xE: // 1110nnnniiiiiiii mov    rn, imm
      return string_printf("mov     r%hhu, ", op_get_r1(op)) + dasm_imm(op_get_simm8(op));

    case 0xF: {
      char size_ch = s.double_precision ? 'd' : 's';
      switch (op_get_r3(op)) {
        case 0x0:
        case 0x1:
        case 0x2:
        ...
        case 0x8: {
          ...
          if (s.double_precision) {
            if (op & 0x0100) {
              return string_printf("fmov    xd%hhu, [r%hhu%s]", op_get_r1(op) & 0xE, op_get_r2(op), suffix);
            } else {
        ...
        case 0xA: {
          ...
          if (s.double_precision) {
            if (op & 0x0100) {
              return string_printf("fmov    [r%hhu%s], xd%hhu", op_get_r1(op), suffix, op_get_r2(op) & 0xE);
            } else {
        ...
        case 0x9:
          ...
          if (s.double_precision) {
            if (op & 0x0100) {
              return string_printf("fmov    xd%hhu, [r%hhu]+", op_get_r1(op) & 0xE, op_get_r2(op));
            } else {
        ...
        case 0xB:
          ...
          if (s.double_precision) {
            if (op & 0x0100) {
              return string_printf("fmov    -[r%hhu], xd%hhu", op_get_r1(op), op_get_r2(op) & 0xE);
            } else {
        ...
        case 0xD:
          switch (op_get_r2(op)) {
            ...
            case 0x2:
              ...
              return string_printf("flds    %cr%hhu, fpul", s.double_precision ? 'd' : 'f', op_get_r1(op));
            case 0x3:
            ...
            case 0xE: // 1111nnmm11101101 fipr   fvn, fvm  # fs(n+3) = dot(fvn, fvm)
              return string_printf("fipr    fv%hhu, fv%hhu", op_get_r1(op) >> 2, op_get_r1(op) & 3);
            case 0xF: {
              uint8_t reg1 = op_get_r1(op);
              if ((reg1 & 0x3) == 0x1) {
                // 1111nn0111111101 ftrv   fvn, xmtrx
                return string_printf("ftrv    fv%hhu, xmtrx", reg1 >> 2);
              } else if (reg1 == 0x3) {
...
uint16_t SH4Emulator::Assembler::asm_bs_calls(const StreamItem& si) const {
  ...
  if (si.arg_types_match({ArgType::PC_MEMORY_REFERENCE})) {
    // 1010dddddddddddd bs     (pc + 4 + 2 * d)
    // 1011dddddddddddd calls  (pc + 4 + 2 * d)
    int32_t delta = this->label_offsets.at(si.args[0].label_name) - (si.offset + 4);
    if ((delta & 0xFFFFF001) != 0 && (delta & 0xFFFFF001) != 0xFFFFF000) {
      throw runtime_error("invalid branch target");
    }
    return asm_op_imm12(is_calls ? 0xB : 0xA, delta);
...
uint16_t SH4Emulator::Assembler::asm_fipr(const StreamItem& si) const {
  ...
  return asm_op_r1_r2_r3(0xF, si.args[0].reg_num << 2 | si.args[1].reg_num, 0xE, 0xD);
}
...
uint16_t SH4Emulator::Assembler::asm_fsts(const StreamItem& si) const {
  ...
  return asm_op_r1_r2_r3(0xF, si.args[1].reg_num, 0x0, 0xD);
}
...
uint16_t SH4Emulator::Assembler::asm_fmov_fmov_s(const StreamItem& si) const {
  ...
  bool reg1 = si.args[0].reg_num | ((si.args[0].type == ArgType::XD_REGISTER) ? 1 : 0);
  bool reg2 = si.args[1].reg_num | ((si.args[1].type == ArgType::XD_REGISTER) ? 1 : 0);
...
uint16_t SH4Emulator::Assembler::asm_fmul(const StreamItem& si) const {
  ...
  // 1111nnnnmmmm0010 fmul   frn, frm
  si.check_2_same_float_regs();
  return asm_op_r1_r2_r3(0xF, si.args[0].reg_num, si.args[1].reg_num, 0xD);
}
...
uint16_t SH4Emulator::Assembler::asm_fneg(const StreamItem& si) const {
  ...
  // 1111nnnn01001101 fneg   frn
  si.check_arg_types({ArgType::FR_DR_REGISTER});
  return asm_op_r1_r2_r3(0xF, si.args[0].reg_num, 0x9, 0xD);
}
...
uint16_t SH4Emulator::Assembler::asm_fsqrt(const StreamItem& si) const {
  ...
  // 1111nnnn01101101 fsqrt  frn
  si.check_arg_types({ArgType::FR_DR_REGISTER});
  return asm_op_r1_r2_r3(0xF, si.args[0].reg_num, 0x3, 0xD);
}
...
uint16_t SH4Emulator::Assembler::asm_mov_b_w_l(const StreamItem& si) const {
  ...
  if (si.arg_types_match({ArgType::MEMORY_REFERENCE, ArgType::INT_REGISTER})) {
    ...
    return asm_op_r1_imm8(0xC, size, si.args[0].value);
    ...
    return asm_op_r1_imm8(0xC, 4 | size, si.args[1].value);
    ...
    int32_t delta;
    if (size == 1) {
      delta = this->label_offsets.at(si.args[0].label_name) - (si.offset + 4);
    } else if (size == 2) {
      delta = this->label_offsets.at(si.args[0].label_name) - ((si.offset & (~3)) + 4);
    } else {
      throw runtime_error("invalid operand size");
    }
    if (delta & ((1 << size) - 1)) {
...
uint16_t SH4Emulator::Assembler::asm_mova(const StreamItem& si) const {
  ...
  int32_t delta = this->label_offsets.at(si.args[0].label_name) - ((si.offset & (~3)) + 4);
  check_range_t(delta, -0x80 * 4, 0x7F * 4);
  return asm_op_r1_imm8(0xC, 0x7, delta);
}
