...
EmulatorBase::EmulatorBase(std::shared_ptr<MemoryContext> mem)
    : mem(mem), instructions_executed(0), log_memory_access(false) {}

void EmulatorBase::set_behavior_by_name(const std::string&) {
  throw std::logic_error("this CPU engine does not implement multiple behaviors");
}

std::vector<EmulatorBase::MemoryAccess> EmulatorBase::get_and_clear_memory_access_log() {
