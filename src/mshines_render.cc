...
struct MonkeyShinesRoom {
  struct EnemyEntry {
    phosg::be_uint16_t y_pixels;
    phosg::be_uint16_t x_pixels;
    phosg::be_int16_t y_min;
    phosg::be_int16_t x_min;
    phosg::be_int16_t y_max;
    phosg::be_int16_t x_max;
    phosg::be_int16_t y_speed; // in pixels per frame
    phosg::be_int16_t x_speed; // in pixels per frame
  ...
  struct BonusEntry {
    phosg::be_uint16_t y_pixels;
    phosg::be_uint16_t x_pixels;
    phosg::be_int32_t unknown[3]; // these appear to be unused
    phosg::be_int16_t type;
    phosg::be_uint16_t id;
  } __attribute__((packed));

  phosg::be_uint16_t enemy_count;
  phosg::be_uint16_t bonus_count;
  EnemyEntry enemies[10];
  BonusEntry bonuses[25];
  phosg::be_uint16_t tile_ids[0x20 * 0x14]; // width=32, height=20
  phosg::be_uint16_t player_start_y; // unused except in rooms 1000 and 10000
  phosg::be_uint16_t player_start_x; // unused except in rooms 1000 and 10000
  phosg::be_uint16_t background_ppat_id;
} __attribute__((packed));

struct MonkeyShinesWorld {
  phosg::be_uint16_t num_exit_keys;
  phosg::be_uint16_t num_bonus_keys;
  phosg::be_uint16_t num_bonuses;
  phosg::be_int16_t exit_door_room;
  phosg::be_int16_t bonus_door_room;
  ...
  // 4 - fall
  // 5 - monster
  phosg::be_uint16_t hazard_types[16];
  ...
  // 20 - death by lava
  phosg::be_uint16_t hazard_death_sounds[16];
  // Explosion sounds can be any of the above or 18 (bomb explosion)
  phosg::be_uint16_t hazard_explosion_sounds[16];
} __attribute__((packed));

std::vector<std::unordered_map<int16_t, std::pair<int16_t, int16_t>>> generate_room_placement_maps(
    const std::vector<int16_t>& room_ids) {
  std::unordered_set<int16_t> remaining_room_ids(room_ids.begin(), room_ids.end());

  // The basic idea is that when Bonzo moves right or left out of a room, the room number is increased or decreased by
  // 1; when he moves up or down out of a room, it's increased or decreased by 100. There's no notion of rooms linking
  // to each other; links are stored implicitly by the room IDs (resource IDs). To convert this format into something
  // we can actually use, we have to find all the connected components of this graph. This recursive lambda adds a
  // single room to a placement map, then uses the flood-fill algorithm to find all the rooms it's connected to.
  std::function<void(std::unordered_map<int16_t, std::pair<int16_t, int16_t>>&, int16_t room_id, int16_t x_offset, int16_t y_offset)> process_room = [&](std::unordered_map<int16_t, std::pair<int16_t, int16_t>>& ret, int16_t room_id, int16_t x_offset, int16_t y_offset) {
    if (remaining_room_ids.erase(room_id)) {
      ret.emplace(room_id, std::make_pair(x_offset, y_offset));
  ...
  // This function generates a placement map with nonnegative offsets that contains the given room
  std::vector<std::unordered_map<int16_t, std::pair<int16_t, int16_t>>> ret;
  ...
  // Start at room 1000 (for main level) and 10000 (for bonus level) and go outward. Both of these start room IDs seem
  // to be hardcoded
  ...
  while (!remaining_room_ids.empty()) {
    ...
    if (remaining_room_ids.size() >= starting_size) {
      throw std::logic_error("did not make progress generating room placement maps");
    }
  }
...
int main(int argc, char** argv) {
  ...
  auto rf = ResourceDASM::parse_resource_fork(phosg::load_file(filename + "/..namedfork/rsrc"));
  ...
  std::unordered_map<int16_t, std::pair<std::shared_ptr<const phosg::ImageRGBA8888N>, size_t>> enemy_image_locations;
  {
    size_t next_type_id = 0;
    for (int16_t id = 1000;; id++) {
      if (!rf.resource_exists(ResourceDASM::RESOURCE_TYPE_PICT, id)) {
        break;
      }
      auto pict = rf.decode_PICT(id);
      auto img = std::make_shared<phosg::ImageRGBA8888N>(std::move(pict.image));
      for (size_t z = 0; z < img->get_height(); z += 80) {
        enemy_image_locations.emplace(next_type_id, std::make_pair(img, z));
        next_type_id++;
      }
    }
  }

  // Decode the default ppat (we'll use it if a room references a missing ppat, which apparently happens quite a lot;
  // it looks like the ppat id field used to be the room id field and they just never updated it after implementing the
  // custom backgrounds feature)
  std::unordered_map<int16_t, const phosg::ImageRGB888> background_ppat_cache;
  auto emplace_ret = background_ppat_cache.emplace(1000, rf.decode_ppat(1000).pattern);
  const phosg::ImageRGB888* default_background_ppat = &emplace_ret.first->second;
  ...
  for (const auto& placement_map : placement_maps) {
    ...
    phosg::ImageRGB888 result(20 * 32 * w_rooms, 20 * 20 * h_rooms);
    ...
    for (auto it : placement_map) {
      ...
      std::string room_data = rf.get_resource(room_type, room_id)->data;
      if (room_data.size() != sizeof(MonkeyShinesRoom)) {
        phosg::fwrite_fmt(stderr, "warning: room {} is not the correct size (expected {} bytes, got {} bytes)\n",
      ...
      // Render the appropriate ppat in the background of every room. We don't use ImageWithoutAlpha::copy_from() here
      // just in case the room dimensions aren't a multiple of the ppat dimensions
      const phosg::ImageRGB888* background_ppat = nullptr;
      try {
        ...
        try {
          ...
          phosg::fwrite_fmt(stderr, "warning: room {} uses ppat {} but it can\'t be decoded ({})\n",
      ...
      for (size_t y = 0; y < 20; y++) {
        for (size_t x = 0; x < 32; x++) {
          ...
          if (tile_x == 0xFFFFFFFF || tile_y == 0xFFFFFFFF) {
            ...
            phosg::fwrite_fmt(stderr, "warning: no known tile for {:02X} (room {}, x={}, y={})\n", tile_id, room_id, x, y);
          } else {
      ...
      for (size_t z = 0; z < room->enemy_count; z++) {
        // It looks like the y coords are off by 80 pixels because of the HUD, which renders at the top. High-quality
        // engineering!
        ...
        result.draw_horizontal_line(room_px + x_min, room_px + x_max, room_py + y_min, 0, 0xFF8000FF);
        result.draw_horizontal_line(room_px + x_min, room_px + x_max, room_py + y_max, 0, 0xFF8000FF);
        result.draw_vertical_line(room_px + x_min, room_py + y_min, room_py + y_max, 0, 0xFF8000FF);
        result.draw_vertical_line(room_px + x_max, room_py + y_min, room_py + y_max, 0, 0xFF8000FF);
        ...
        if (room->enemies[z].x_speed || room->enemies[z].y_speed) {
          ...
          result.draw_line(enemy_px + 20, enemy_py + 20, enemy_px + 20 + room->enemies[z].x_speed * 10,
      ...
      // If this is a starting room, mark the player start location with an arrow and the label "START"
