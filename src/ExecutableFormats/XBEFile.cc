...
void XBEFile::parse() {
  this->r = phosg::StringReader(this->data);
...
uint32_t XBEFile::entrypoint_addr() const {
  std::array<uint32_t, 3> keys = {0xE682F45B, 0x94859D4B, 0xA8FC57AB};
...
uint32_t XBEFile::kernel_thunk_table_addr() const {
  std::array<uint32_t, 3> keys = {0x46437DCD, 0xEFB1F152, 0x5B6D40B6};
  for (uint32_t key : keys) {
...
void XBEFile::print(
  ...
  phosg::fwrite_fmt(stream, "[XBE file: {}]\n", this->filename);
  phosg::fwrite_fmt(stream, "  code signature: {}\n", phosg::format_data_string(this->header->code_signature, sizeof(this->header->code_signature)));
  phosg::fwrite_fmt(stream, "  base_addr: {:08X}\n", this->header->base_addr);
  phosg::fwrite_fmt(stream, "  header_size: {:08X}\n", this->header->header_size);
  phosg::fwrite_fmt(stream, "  image_size: {:08X}\n", this->header->image_size);
  phosg::fwrite_fmt(stream, "  image_header_size: {:08X}\n", this->header->image_header_size);
  phosg::fwrite_fmt(stream, "  creation_time: {:08X}\n", this->header->creation_time);
  phosg::fwrite_fmt(stream, "  certificate_addr: {:08X}\n", this->header->certificate_addr);
  phosg::fwrite_fmt(stream, "  num_sections: {:08X}\n", this->header->num_sections);
  phosg::fwrite_fmt(stream, "  section_headers_addr: {:08X}\n", this->header->section_headers_addr);
  phosg::fwrite_fmt(stream, "  init_flags: {:08X}\n", this->header->init_flags);
  phosg::fwrite_fmt(stream, "  entrypoint_addr: {:08X} (decoded: {:08X})\n", this->header->entrypoint_addr_encoded, this->entrypoint_addr());
  phosg::fwrite_fmt(stream, "  tls_addr: {:08X}\n", this->header->tls_addr);
  phosg::fwrite_fmt(stream, "  stack_size: {:08X}\n", this->header->stack_size);
  phosg::fwrite_fmt(stream, "  pe_heap_reserve: {:08X}\n", this->header->pe_heap_reserve);
  phosg::fwrite_fmt(stream, "  pe_heap_commit: {:08X}\n", this->header->pe_heap_commit);
  phosg::fwrite_fmt(stream, "  pe_base_addr: {:08X}\n", this->header->pe_base_addr);
  phosg::fwrite_fmt(stream, "  pe_size: {:08X}\n", this->header->pe_size);
  phosg::fwrite_fmt(stream, "  pe_checksum: {:08X}\n", this->header->pe_checksum);
  phosg::fwrite_fmt(stream, "  pe_creation_time: {:08X}\n", this->header->pe_creation_time);
  phosg::fwrite_fmt(stream, "  debug_path_addr: {:08X}\n", this->header->debug_path_addr);
  phosg::fwrite_fmt(stream, "  debug_filename_addr: {:08X}\n", this->header->debug_filename_addr);
  phosg::fwrite_fmt(stream, "  utf16_debug_filename_addr: {:08X}\n", this->header->utf16_debug_filename_addr);
  phosg::fwrite_fmt(stream, "  kernel_thunk_table_addr: {:08X} (decoded: {:08X})\n", this->header->kernel_thunk_table_addr_encoded, this->kernel_thunk_table_addr());
  phosg::fwrite_fmt(stream, "  import_directory_addr: {:08X}\n", this->header->import_directory_addr);
  phosg::fwrite_fmt(stream, "  num_library_versions: {:08X}\n", this->header->num_library_versions);
  phosg::fwrite_fmt(stream, "  library_versions_addr: {:08X}\n", this->header->library_versions_addr);
  phosg::fwrite_fmt(stream, "  kernel_library_version_addr: {:08X}\n", this->header->kernel_library_version_addr);
  phosg::fwrite_fmt(stream, "  xapi_library_version_addr: {:08X}\n", this->header->xapi_library_version_addr);
  phosg::fwrite_fmt(stream, "  logo_bitmap_addr: {:08X}\n", this->header->logo_bitmap_addr);
  phosg::fwrite_fmt(stream, "  logo_bitmap_size: {:08X}\n", this->header->logo_bitmap_size);
  phosg::fwrite_fmt(stream, "  unknown_a1: {:016X}\n", this->header->unknown_a1);
  phosg::fwrite_fmt(stream, "  unknown_a2: {:08X}\n", this->header->unknown_a2);

  std::multimap<uint32_t, std::string> all_labels;
  ...
  for (size_t x = 0; x < this->sections.size(); x++) {
    ...
    auto content_sha1 = phosg::SHA1(sec_data, sec.file_size).bin();
    ...
    phosg::fwrite_fmt(stream, "  name: {}\n", this->r.pget_cstr(sec.name_addr - this->base_addr));
    phosg::fwrite_fmt(stream, "  flags: {:08X}\n", sec.flags);
    phosg::fwrite_fmt(stream, "  addr: {:08X}\n", sec.addr);
    phosg::fwrite_fmt(stream, "  size: {:08X}\n", sec.size);
    phosg::fwrite_fmt(stream, "  file_offset: {:08X}\n", sec.file_offset);
    phosg::fwrite_fmt(stream, "  file_size: {:08X}\n", sec.file_size);
    phosg::fwrite_fmt(stream, "  reference_index: {:08X}\n", sec.reference_index);
    phosg::fwrite_fmt(stream, "  head_reference_addr: {:08X}\n", sec.head_reference_addr);
    phosg::fwrite_fmt(stream, "  tail_reference_addr: {:08X}\n", sec.tail_reference_addr);
    auto sha1_str = phosg::format_data_string(sec.content_sha1, sizeof(sec.content_sha1));
    if (sha1_correct) {
      phosg::fwrite_fmt(stream, "  content_sha1: {} (correct)\n", sha1_str);
    } else {
      phosg::fwrite_fmt(stream, "  content_sha1: {} (expected {})\n", sha1_str, phosg::format_data_string(content_sha1));
    }

    if (sec.file_size) {
      if (all_sections_as_code || sec.flags & 0x00000004) {
        std::string disassembly = X86Emulator::disassemble(sec_data, sec.file_size, sec.addr, &all_labels);
...
phosg::StringReader XBEFile::read_from_addr(uint32_t addr, uint32_t size) const {
  ...
  throw std::out_of_range("address not within header or any section");
}
