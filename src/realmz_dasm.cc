...
int disassemble_scenario(
    const ResourceDASM::RealmzScenarioData& scen,
  ...
  {
    std::string filename = std::format("{}/script.txt", out_dir);
    auto f = phosg::fopen_unique(filename, "wt");

    phosg::fwritex(f.get(), scen.disassemble_global_metadata());
    phosg::log_info_f("... {} (global metadata)", filename);

    phosg::fwritex(f.get(), scen.disassemble_scenario_metadata());
    phosg::log_info_f("... {} (scenario metadata)", filename);

    phosg::fwritex(f.get(), scen.disassemble_restrictions());
    phosg::log_info_f("... {} (restrictions)", filename);

    phosg::fwritex(f.get(), scen.disassemble_solids());
    ...
    phosg::fwritex(f.get(), scen.disassemble_all_monsters());
    phosg::log_info_f("... {} (monsters)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_battles());
    phosg::log_info_f("... {} (battles)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_custom_item_definitions());
    phosg::log_info_f("... {} (items)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_shops());
    phosg::log_info_f("... {} (shops)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_treasures());
    phosg::log_info_f("... {} (treasures)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_party_maps());
    phosg::log_info_f("... {} (party maps)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_simple_encounters());
    phosg::log_info_f("... {} (simple encounters)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_complex_encounters());
    phosg::log_info_f("... {} (complex encounters)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_rogue_encounters());
    phosg::log_info_f("... {} (rogue encounters)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_time_encounters());
    phosg::log_info_f("... {} (time encounters)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_level_aps_and_rrs(true));
    phosg::log_info_f("... {} (dungeon APs and RRs)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_level_aps_and_rrs(false));
    phosg::log_info_f("... {} (land APs and RRs)", filename);

    phosg::fwritex(f.get(), scen.disassemble_all_xaps());
  ...
  // Save media
  for (int16_t id : scen.scenario_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_PICT)) {
    auto decoded = scen.scenario_rsf.decode_PICT(id);
    std::string filename = std::format("{}/media/picture_{}", out_dir, id);
    filename = image_saver->save_image(decoded.image, filename);
    phosg::log_info_f("... {}", filename);
  }
  for (int16_t id : scen.scenario_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_cicn)) {
    auto decoded = scen.scenario_rsf.decode_cicn(id);
    std::string filename = std::format("{}/media/icon_{}", out_dir, id);
    filename = image_saver->save_image(decoded.image, filename);
    phosg::log_info_f("... {}", filename);
  }
  ...
  for (int16_t id : scen.scenario_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_TEXT)) {
    try {
      if (!scen.scenario_rsf.resource_exists(ResourceDASM::RESOURCE_TYPE_styl, id)) {
        throw std::runtime_error("TEXT resource has no corresponding styl resource");
      }
      std::string filename = std::format("{}/media/text_{}.rtf", out_dir, id);
      phosg::save_file(filename, scen.scenario_rsf.decode_styl(id));
      phosg::log_info_f("... {}", filename);
    } catch (const std::runtime_error& e) {
      std::string filename = std::format("{}/media/text_{}.txt", out_dir, id);
      phosg::save_file(filename, scen.scenario_rsf.decode_TEXT(id));
  ...
  for (auto it : scen.land_type_to_tileset_definition) {
    ...
    std::string filename = std::format("{}/tileset_{}_legend", out_dir, it.first);
    int16_t resource_id = scen.global.pict_resource_id_for_land_type(it.first);
    if (!scen.scenario_rsf.resource_exists(ResourceDASM::RESOURCE_TYPE_PICT, resource_id)) {
      phosg::log_info_f("### {} FAILED: PICT {} is missing", filename, resource_id);
    } else {
      phosg::ImageRGBA8888N positive_pattern = scen.scenario_rsf.decode_PICT(resource_id).image;
      phosg::ImageRGB888 legend = scen.global.generate_tileset_definition_legend(it.second, positive_pattern);
  ...
  for (size_t z = 0; z < scen.dungeon_maps.size(); z++) {
    std::string filename = std::format("{}/dungeon_{}", out_dir, z);
    if (generate_maps_as_json) {
      filename += ".json";
      std::string s = scen.generate_dungeon_map_json(z);
      phosg::save_file(filename, s);
      phosg::log_info_f("... {}", filename);
    } else {
      phosg::ImageRGB888 map = scen.generate_dungeon_map(z, 0, 0, 90, 90, show_random_rects);
  ...
  std::unordered_map<std::string, std::unordered_set<uint8_t>> used_positive_tiles;
  for (size_t z = 0; z < scen.land_maps.size(); z++) {
    std::string filename = std::format("{}/land_{}", out_dir, z);
    try {
      if (generate_maps_as_json) {
        filename += ".json";
        std::string s = scen.generate_land_map_json(z);
        phosg::save_file(filename, s);
        phosg::log_info_f("... {}", filename);
      } else {
        phosg::ImageRGB888 map = scen.generate_land_map(
  ...
  for (size_t z = 0; z < scen.party_maps.size(); z++) {
    std::string filename = std::format("{}/map_{}", out_dir, z);
    try {
      phosg::ImageRGB888 map = scen.render_party_map(z);
      filename = image_saver->save_image(map, filename);
      phosg::log_info_f("... {}", filename);
    } catch (const std::exception& e) {
      phosg::log_info_f("### {} FAILED: {}", filename, e.what());
  ...
  for (auto layout_component : scen.layout.get_connected_components()) {
    ...
    std::string filename = std::format("{}/land_connected", out_dir);
    for (int y = 0; y < 8; y++) {
      for (int x = 0; x < 16; x++) {
    ...
    phosg::ImageRGB888 connected_map = scen.generate_layout_map(layout_component, show_random_rects);
  ...
  if (show_unused_tile_ids) {
    ...
    for (int16_t z : scen.scenario_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_cicn)) {
...
int disassemble_saved_game(
    const ResourceDASM::RealmzSaveData& save,
    const std::string& out_dir,
    const ResourceDASM::ImageSaver* image_saver) {
  // Make necessary directories for output
  std::filesystem::create_directories(out_dir);
  ...
  // Disassemble scenario text
  {
    ...
    phosg::fwritex(f.get(), save.disassemble_game_state());
    phosg::log_info_f("... {} (game state)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_shops());
    phosg::log_info_f("... {} (shops)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_simple_encounters());
    phosg::log_info_f("... {} (simple encounters)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_complex_encounters());
    phosg::log_info_f("... {} (complex encounters)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_rogue_encounters());
    phosg::log_info_f("... {} (rogue encounters)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_time_encounters());
    phosg::log_info_f("... {} (time encounters)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_land_level_states());
    phosg::log_info_f("... {} (dungeon APs and RRs)", filename);

    phosg::fwritex(f.get(), save.disassemble_all_dungeon_level_states());
    phosg::log_info_f("... {} (land APs and RRs)", filename);
  }
  ...
  for (size_t z = 0; z < save.dungeon_level_states.size(); z++) {
    std::string filename = std::format("{}/dungeon_{}", out_dir, z);
    try {
      phosg::ImageRGB888 map = save.generate_dungeon_map(z, 0, 0, 90, 90);
      filename = image_saver->save_image(map, filename);
      phosg::log_info_f("... {}", filename);
    } catch (const std::exception& e) {
      phosg::log_info_f("### {} FAILED: {}", filename, e.what());
  ...
  for (size_t z = 0; z < save.land_level_states.size(); z++) {
    std::string filename = std::format("{}/land_{}", out_dir, z);
    try {
      phosg::ImageRGB888 map = save.generate_land_map(z, 0, 0, 90, 90);
      filename = image_saver->save_image(map, filename);
      phosg::log_info_f("... {}", filename);
    } catch (const std::exception& e) {
      phosg::log_info_f("### {} FAILED: {}", filename, e.what());
  ...
  for (auto layout_component : save.scenario.layout.get_connected_components()) {
    ...
    phosg::ImageRGB888 connected_map = save.generate_layout_map(layout_component);
    filename = image_saver->save_image(connected_map, filename);
    phosg::log_info_f("... {}", filename);
  }
...
int disassemble_global_data(
    ResourceDASM::RealmzGlobalData& global, const std::string& out_dir, const ResourceDASM::ImageSaver* image_saver) {
  // Make necessary directories for output
  std::filesystem::create_directories(out_dir);
  if (image_saver) {
    std::filesystem::create_directories(std::format("{}/media", out_dir));
  ...
  // Disassemble non-media data
  {
    ...
    phosg::save_file(filename, global.disassemble_all_caste_definitions());
    phosg::log_info_f("... {}", filename);

    filename = std::format("{}/races.txt", out_dir);
    phosg::save_file(filename, global.disassemble_all_race_definitions());
    phosg::log_info_f("... {}", filename);

    filename = std::format("{}/items.txt", out_dir);
    phosg::save_file(filename, global.disassemble_all_item_definitions());
    phosg::log_info_f("... {}", filename);

    filename = std::format("{}/spells.txt", out_dir);
    phosg::save_file(filename, global.disassemble_all_spell_definitions());
    phosg::log_info_f("... {}", filename);
  }
  ...
  for (int16_t id : global.global_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_PICT)) {
    auto decoded = global.global_rsf.decode_PICT(id);
    std::string filename = std::format("{}/media/picture_{}", out_dir, id);
    filename = image_saver->save_image(decoded.image, filename);
    phosg::log_info_f("... {}", filename);
  }
  for (int16_t id : global.global_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_cicn)) {
    auto decoded = global.global_rsf.decode_cicn(id);
    std::string filename = std::format("{}/media/icon_{}", out_dir, id);
    filename = image_saver->save_image(decoded.image, filename);
    phosg::log_info_f("... {}", filename);
  }
  for (int16_t id : global.portraits_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_cicn)) {
    auto decoded = global.portraits_rsf.decode_cicn(id);
    std::string filename = std::format("{}/media/portrait_{}", out_dir, id);
    filename = image_saver->save_image(decoded.image, filename);
    phosg::log_info_f("... {}", filename);
  }
  for (int16_t id : global.tacticals_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_cicn)) {
    auto decoded = global.tacticals_rsf.decode_cicn(id);
    std::string filename = std::format("{}/media/battle_icon_{}", out_dir, id);
    filename = image_saver->save_image(decoded.image, filename);
    phosg::log_info_f("... {}", filename);
  }
  ...
  for (int16_t id : global.global_rsf.all_resources_of_type(ResourceDASM::RESOURCE_TYPE_TEXT)) {
    try {
      if (!global.global_rsf.resource_exists(ResourceDASM::RESOURCE_TYPE_styl, id)) {
        throw std::runtime_error("TEXT resource has no corresponding styl resource");
      }
      std::string filename = std::format("{}/media/text_{}.rtf", out_dir, id);
      phosg::save_file(filename, global.global_rsf.decode_styl(id));
      phosg::log_info_f("... {}", filename);
    } catch (const std::runtime_error& e) {
      std::string filename = std::format("{}/media/text_{}.txt", out_dir, id);
      phosg::save_file(filename, global.global_rsf.decode_TEXT(id));
      phosg::log_info_f("*** {} (style rendering failed: {})", filename, e.what());
    }
  }
  ...
  for (auto it : global.land_type_to_tileset_definition) {
    std::string filename = std::format("{}/tileset_{}_legend",
        out_dir, it.first);
    int16_t resource_id = global.pict_resource_id_for_land_type(it.first);
    phosg::ImageRGBA8888N positive_pattern = global.global_rsf.decode_PICT(resource_id).image;
    phosg::ImageRGB888 legend = global.generate_tileset_definition_legend(it.second, positive_pattern);
    filename = image_saver->save_image(legend, filename);
    phosg::log_info_f("... {}", filename);
  }
...
int main(int argc, char** argv) {
  ...
  for (int x = 1; x < argc; x++) {
    if (image_saver.process_cli_arg(argv[x])) {
      ...
      parallelism = std::stoll(&argv[x][11], nullptr, 0);
  ...
  if (parallelism >= 0) {
    ...
    ResourceDASM::RealmzGlobalData global(global_data_dir);
    ...
    auto disassemble_item = [&](const std::string& scen_name, size_t) -> bool {
      if (scen_name.empty()) {
        ...
        ResourceDASM::RealmzScenarioData scen(global, std::format("{}/{}", scenarios_dir, scen_name), scen_name);
    ...
    ResourceDASM::RealmzGlobalData global(data_dir);
    ...
    std::string scenario_name = (slash_pos == std::string::npos) ? scenario_dir : scenario_dir.substr(slash_pos + 1);

    ResourceDASM::RealmzGlobalData global(data_dir);
    ResourceDASM::RealmzScenarioData scen(global, scenario_dir, scenario_name);
    ...
    if (out_dir.empty()) { // Disassembling a scenario
      ...
      ResourceDASM::RealmzSaveData save(scen, save_dir);
