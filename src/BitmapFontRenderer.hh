...
public:
  ...
  // after the last hyphen. If there were no spaces or hyphens, the word is
  // broken as close to max_width as possible.
  std::string wrap_text_to_pixel_width(const std::string& text, size_t max_width);

  // Computes the width and height of the area required to render all of the
  // given text.
  std::pair<size_t, size_t> pixel_dimensions_for_text(const std::string& text);
  ...
  void render_text(Image& canvas, const std::string& text, ssize_t x, ssize_t y, uint32_t color);
  ...
  void render_text_custom(const std::string& text, std::function<void(size_t, size_t)> write_pixel);
