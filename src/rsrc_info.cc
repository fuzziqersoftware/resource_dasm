...
static uint32_t calc_resource_size_of_type(const ResourceFile& file, uint32_t type) {
  uint32_t result = 0;
...
static uint32_t calc_resource_size_of_all_types(const ResourceFile& file) {
  uint32_t result = 0;
...
int main(int argc, const char** argv) {
  try {
    ...
    for (const char* i : input_filenames) {
      ...
      ResourceFile input_file = parse_resource_fork(load_file(input_filename));
      ...
      printf("File '%s':\n", input_filename.c_str());
      {
        ...
        for (uint32_t res_type : all_types) {
          ...
          printf("  %s: %4u (%7u bytes)\n", string_for_resource_type(res_type).c_str(), count, size);
        }
        printf("  ----------\n");
